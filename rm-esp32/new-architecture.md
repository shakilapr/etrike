...
#### 1. Steering Actuator Conversion (SES / SES on `0x169 VCU_SES_REQ`)
...
4. **ECU Internal Inverse Decoding** (executed inside the SES microcontroller):
...
* **Actuator Hardware Safety Window**: The SES actuator firmware defines its absolute fault threshold at $\pm 70.0^\circ$ (raw $23000 \dots 37000$). Any frame with raw $< 23000$ or $> 37000$ immediately sets `SES_OverAngle_Err` and shuts off motor assist.
...
│      * 0x169: can::custom::ses::Command (Steering SES)  │
