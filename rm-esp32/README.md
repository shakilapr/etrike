...
- **Steering Control (`0x169 VCU_SES_REQ`)**:
  - Right gimbal horizontal axis $\longrightarrow$ proportional steering angle $\pm 450.0^\circ$ to EPS-C / SES.
...
| **GPIO 18** | RC CH0 | Input | Steering Right Gimbal Horizontal ($\pm 450.0^\circ$) |
| **GPIO 19** | RC CH1 | Input | Brake Left Gimbal Vertical ($0\dots 27\text{ mm}$) |
| **GPIO 14** | RC CH2 | Input | VRA Rotary Dial (Speed Limiter / Trim) |
| **GPIO 32** | RC CH3 | Input | VRB Rotary Dial (Aux Expansion) |
| **GPIO 13** | RC CH4 | Input | SWB 2-Position Toggle (Ignition Enable) |
| **GPIO 4** | RC CH5 | Input | SWC 3-Position Toggle (Gear: Reverse / Neutral / Drive) |
...
└── test/
    └── test_rm_receiver.cpp # Native unit test suite (36 assertions)
```
