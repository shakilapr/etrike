...
│  Jetson  │◄───────────────────────────►│  RT ESP32-S3 │◄──────────────────►│ SYS ESP32-S3 │◄──────────────►│ SES/SEB  │
│  Orin NX │                             │              │  (UART or SPI)     │              │  (500 kbit/s)  │ SES    │
│          │                             │              │  framed + CRC-8   │              │                │ SEB      │
└──────────┘                             └──────────────┘                    └──────────────┘                └──────────┘
```
...
- **Private CAN**: SYS ↔ SES/SEB SES (steering) and SEB (brake). Not visible to Jetson or RT.
...
### 5.2 Private CAN inputs (from SES/SEB actuators)

| CAN ID | Signal | Type | Rate | Notes |
|--------|--------|------|------|-------|
| `0x201` | `SES/SEBEpsStatus` | raw 8 bytes | ~50 Hz | SES steering feedback |
| `0x721` | `SES/SEBSebStatus` | raw 8 bytes | ~50 Hz | SEB brake feedback |
...
### 6.2 Private CAN outputs (to SES/SEB actuators)

| CAN ID | Signal | Type | Rate | Notes |
|--------|--------|------|------|-------|
| `0x169` | `SES/SEBEpsCommand` | raw 8 bytes | 50 Hz | SES steering target |
| `0x7B0` | `SES/SEBSebCommand` | raw 8 bytes | 50 Hz | SEB brake target |
...
Steering (AUTO, via SES/SEB private CAN):
  RtToSysSetpoint.steer_angle_mdeg ──► SES/SEBEpsCommand ──► CAN 0x169

Brake (AUTO, via SES/SEB private CAN):
  RtToSysSetpoint.brake_pressure_kpa ──► SES/SEBSebCommand ──► CAN 0x7B0
```
...
| 4 | 4 B | `actual_steer_angle_mdeg` | `int32_t` | SES feedback [+right] |
| 8 | 4 B | `brake_pressure_kpa` | `int32_t` | SEB feedback |
| 12 | 2 B | `SES/SEB_fault_bits` | `uint16_t` | Aggregated fault flags |
...
### Private bus (SYS ↔ SES/SEB)

| ID | Name | Dir | DLC | Notes |
|----|------|-----|-----|-------|
| `0x169` | SESOMMAND | SYS→SES | 8 | 20 ms period |
| `0x201` | EPS_STATUS | SES→SYS | 8 | Feedback |
...
- **Auto**: Jetson → CAN `0x300` → RT physics → inter-MCU `0x10` → SYS motor + SES + SEB.
...
| `can_tx` | 2 | 5 Hz | — | Private CAN SES + SEB commands |
