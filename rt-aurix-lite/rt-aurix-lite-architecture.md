...
| `0x169` | VCU_SES_REQ | AURIX | SES | 8 | 50 Hz | Medium |
| `0x201` | SES_STATUS | SES | AURIX | 8 | 100 Hz | Medium |
| `0x202` | SES_ErrInfo | SES | AURIX | 8 | 10 Hz | Medium |
| `0x203` | SES_Version | SES | AURIX | 8 | 1 Hz | Lowest |
...
| `0x6FA` | SES_Test | SES | AURIX | 8 | 100 Hz | Lowest |
...
**Key difference from distributed:** No mode-gated dual control. The AURIX owns both SES (0x169) and SEB (0x7B9) in all modes. In MANUAL: SES standalone, SEB follows brake lever. In AUTO: SES follows kinematics angle, SEB follows brake arbitration. In ESTOP: SES ramps to 0°, SEB goes to max stroke.
...
Steering wheel        ──► SES standalone (AURIX monitors 0x201)
...
5. **All multi-byte CAN fields are big-endian (MSB first)** — unless SES/SEB protocol specifies otherwise. (Same)
...
9. **Listen Before Speaking.** SES/SEB units require status feedback before command. (Same)
10. **EGAS 3-level safety separation for motor actuation.** MTR STM32 isolation preserved. (Same)
11. **Single owner of both SES/SEB actuators.** AURIX commands SES and SEB directly in all modes. No mode-gated dual control needed. No single MCU failure can take both actuators because... there's only one MCU. The MTR STM32 provides motor kill independence. SES and SEB have internal timeout-fault behavior as hardware backup.
...
| SES owner | RT (0x169) | AURIX (0x169) |
