...
```
  ...
  │                                                                  │
  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐        │
  │  │ SES/SEB  │  │ SES/SEB  │  │  DC-DC   │  │   MTR    │        │
  │  │   SEB    │  │  SES   │  │ Converter│  │  STM32   │        │
...
> **Dual CAN hardware on AURIX:** The AURIX TC3xx has multiple MCMCAN modules. MCMCAN0 drives the low-level CAN bus (SES/SEB, MTR, DC-DC). MCMCAN1 drives the high-level CAN bus (Jetson). The `task_can_rx` and `task_can_tx` run per-bus, same as RT's `can_rx_low`/`can_rx_high` in the distributed variant.
...
| Mode | Behavior |
|------|----------|
| **MANUAL** | Rider steers / rides throttle. MTR reads throttle ADC + gear sense → pass-through via MCP4725 + relays. Brake lever → AURIX GPIO → CAN 0x7B9 → SEB (Stroke Mode, 15mm/0mm). SES standalone (AURIX monitors 0x201). DC-DC on. Mode via CAN 0x110 (AURIX → MTR). |
| **AUTO** | Jetson `/cmd_vel` → high CAN 0x300 → AURIX kinematics → low CAN 0x204 (MTR) + 0x169 (SES). Brake via 0x7B9 (Pressure Mode from arbitration, Stroke override for lever). Lights from Jetson via 0x302. |
