...
> drives the low-level bus (SES, SEB, MTR) using the on-board TLE9251VSJ transceiver on
...
### 3.1 Low-Level CAN (500 kbit/s) — nodes: RT, SES, SEB, MTR
...
| `0x202` | SES_ERR_INFO | SES | RT | 8 | 10 Hz |
...
| State | Behavior |
|------|----------|
| **MANUAL** | Rider steers; SES standalone (RT monitors `0x201`). Brake lever → RT GPIO → `0x7B9` (Stroke mode 15 mm/0 mm). MTR pass-through from grip/gear. RT keeps lights on. |
| **AUTO** | Jetson `0x300` → RT kinematics → `0x204` (MTR) + `0x169` (SES). Brake via `0x7B9` (Pressure mode from arbitration; stroke override for lever). Lights from Jetson `0x302`. Optional `0x303` direct steer command → arbitration. |
...
| SES angle implausible | >30° off center at boot sync | Refuse ACTIVE → FAULT |
| SES L3 fault (`0x202`) | L3 fault bits | ESTOP via `0x001` |
...
|-----|---------------------|-----------|
| Low | Full ESTOP: `0x001` both buses | Loss of actuator bus (SES/SEB/MTR) is non-survivable |
