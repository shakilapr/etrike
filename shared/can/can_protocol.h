...
// Low-level CAN (500 kbit/s): RT, SYS, SES, SEB, DC-DC converter.
...
// ╔══════════════════════════════════════════════════════════════════════╗
// ║  SES/SEB CAN IDs — factory-programmed, CANNOT be changed          ║
// ║  Steering: SES.  Brake: SEB.  Source: docs/by-wire-*.csv       ║
// ╚══════════════════════════════════════════════════════════════════════╝

// ── SES/SEB SES (steering) ──────────────────────────────────────

constexpr uint32_t kIdSES/SEBEpsCmd      = 0x169;  // RT→SES: VCU_SES_Req, 50 Hz (factory default)
constexpr uint32_t kIdSES/SEBEpsStatus   = 0x201;  // SES→RT: SES_Status, 100 Hz (factory default)
constexpr uint32_t kIdSES/SEBEpsErrInfo  = 0x202;  // SES→RT: SES_ErrInfo, 100 ms (factory default)
constexpr uint32_t kIdSES/SEBEpsVersion  = 0x203;  // SES→RT: SES_Version, 1000 ms (factory default)
constexpr uint32_t kIdSES/SEBEpsTest     = 0x6FA;  // SES→RT: SES_Test, 100 Hz (factory default)
...
struct SteerDiag {
    ...
    uint8_t  fault;            // byte 2: 0=OK, 1=SES fault
    uint16_t motor_current;    // bytes 3-4: SES motor current (0.01A/bit)
    uint16_t ecu_temp;         // bytes 5-6: SES ECU temperature (0.1°C/bit)
...
// ══ SES/SEB payload structs (factory-fixed, cannot change) ══

// 0x169 VCU_SES_REQ — RT→SES (SES/SEB  // CSV spec, Motorola LSB)
struct VcuSesReq {
...
// 0x7B9 VCU_SEB_REQ — SYS→SEB (brake command, SES/SEB protocol)
...
// ───────────────────────────────────────────────────────────────────
// SES/SEB little-endian pack/unpack (Motorola LSB)
// ───────────────────────────────────────────────────────────────────
...
inline void VcuSesReq::pack(uint8_t raw[8]) const {
    ...
    // Checksum: XOR(bytes 0-6) ^ 0xFF (per SES/SEB CSV spec)
    uint8_t cksum = 0;
...
// 0x201 SES_Status — SES→RT (SES/SEB status frame, per docs/by-wire - steering.csv)
struct SesStatus {
...
// 0x721 SEB_Status — SEB→SYS (SES/SEB status frame, per docs/by-wire - brake.csv)
struct SebStatus {
