...
constexpr uint32_t kIdVcuSesReq         = 0x169;  // RT->SES steering cmd, 50 Hz (SES/SEB factory 0x169)
constexpr uint32_t kIdSesStatus         = 0x201;  // SES→RT steering feedback, 100 Hz
...
constexpr uint32_t kIdVcuSebReq         = 0x7B9;  // SYS->SEB brake cmd, 50 Hz (SES/SEB factory 0x7B9)
...
constexpr auto kIdSES/SEBEpsStatus  = kIdSesStatus;       // → canonical
constexpr auto kIdSES/SEBSebStatus  = kIdSebStatus;       // → canonical
...
// 0x169 VCU_SES_REQ — RT→SES (steering command, SES/SEB protocol)
