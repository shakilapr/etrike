...
constexpr uint32_t kIdSbwCmd      = 0x169;  // RT→SES: VCU_SES_Req, 50 Hz (factory default)
constexpr uint32_t kIdSbwStatus   = 0x201;  // SES→RT: SES_Status, 100 Hz (factory default)
constexpr uint32_t kIdSbwErrInfo  = 0x202;  // SES→RT: SES_ErrInfo, 100 ms (factory default)
constexpr uint32_t kIdSbwVersion  = 0x203;  // SES→RT: SES_Version, 1000 ms (factory default)
constexpr uint32_t kIdSbwTest     = 0x6FA;  // SES→RT: SES_Test, 100 Hz (factory default)
...
// 0x169 VCU_SES_REQ — RT→SES (steer-by-wire  // CSV spec, Motorola LSB)
struct VcuSesReq {
...
constexpr uint8_t kEstopReasonInternal       = 7;  // SEB L3, SES L3 fault, etc.
...
// 0x201 SES_Status — SES→RT (steer-by-wire status frame, per docs/by-wire - steering.csv)
struct SesStatus {
