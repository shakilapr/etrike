...
// Bus 2 (private): SYS ESP32-S3 <-> SES/SEB SES/SEB actuators.
...
// ── Private CAN ID assignments: SYS <-> SES/SEB actuators ───────
constexpr uint32_t kIdSES/SEBEpsCommand = 0x169;  // SYS → SES, 20 ms command
constexpr uint32_t kIdSES/SEBEpsStatus  = 0x201;  // SES → SYS, status feedback
constexpr uint32_t kIdSES/SEBSebCommand = 0x7B0;  // SYS → SEB, 20 ms command
constexpr uint32_t kIdSES/SEBSebStatus  = 0x721;  // SEB → SYS, status feedback
...
// 0x169 SES command on the private SES/SEB bus.
// Byte layout must be verified against the project-specific SES/SEB protocol
// before enabling transmission on hardware.
struct SES/SEBEpsCommand {
    uint8_t raw[8] = {};

    void to_frame(Frame& f) const {
        f.id = kIdSES/SEBEpsCommand; f.dlc = 8; f.extended = false;
        for (int i = 0; i < 8; ++i) f.data[i] = raw[i];
    }
};

// 0x7B0 SEB command on the private SES/SEB bus.
// Byte layout must be verified against the project-specific SES/SEB protocol
// before enabling transmission on hardware.
struct SES/SEBSebCommand {
    uint8_t raw[8] = {};

    void to_frame(Frame& f) const {
        f.id = kIdSES/SEBSebCommand; f.dlc = 8; f.extended = false;
        for (int i = 0; i < 8; ++i) f.data[i] = raw[i];
    }
};
