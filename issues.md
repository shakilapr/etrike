...
**Solution:** Obtain SEB pressure spec from SES/SEB, define raw-to-MPa conversion
...
- Force-activation with target=0° means manual steering only (SES holds centered)
...
Intentional design choice. Motor encoder + SES/SEB feedback provide independent speed
...
| # | Issue | Resolution |
|---|-------|-----------|
| R1 | SES timeout-fault behavior TBD (old Gap #3) | **Resolved.** Two-tier ESTOP steering: obstacle → hold angle then silent-stop; non-obstacle → active ramp to 0° at 20°/s via `0x200`, fallback to silent-stop on mechanical jam. STEER_FAULT now has START-button recovery paths. Gap #3 struck through in §12. |
| R2 | Steering pre-ESTOP sequence missing | **Resolved.** Active centering is now the primary ESTOP path. SES timeout-fault is only a last-resort fallback (CAN bus dead). |
...
| C3 | ~~P0~~ **DONE** | Pressure mode mapping undefined (Gap #4) | ✅ Verified SES/SEB SEB spec: `VCU_SEB_Pre_Value_Req` is u8 at bit 24 (byte 3, mode-muxed with Stroke), scale 0.05 MPa/bit, range 0–5 MPa, raw 0–100. Conversion: `seb_raw = kPa × 0.02`. Both docs updated. Gap #4 struck through. |
