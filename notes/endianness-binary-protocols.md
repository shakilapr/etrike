...
The E-Trike uses **big-endian (MSB first)** for multi-byte CAN fields — except SES/SEB actuators which use **little-endian (LSB first)** per their factory protocol. The `can-dictionary.md` marks endianness on every signal.
...
**The E-Trike convention:** All signals use big-endian (Motorola) unless marked otherwise. SES/SEB protocols use "Motorola LSB" — Motorola bit numbering but little-endian byte order. See the signal tables in `can-dictionary.md`.
...
Most automotive standards (J1939, CANopen) follow this convention and define multi-byte signals as big-endian. The E-Trike does the same for consistency — except where SES/SEB factory protocols dictate little-endian.
...
// For SES/SEB (little-endian), just memcpy if the CPU is little-endian (ESP32 is).
...
- For SES/SEB little-endian signals: use native byte order — no conversion needed.
...
*See also: [[can-protocol]] for CAN frame structure, `can-dictionary.md` for bit-level layouts of every signal, `docs/steering-unit.md` for SES/SEB little-endian layout.*
