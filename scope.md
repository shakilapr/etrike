...
- **Front Steering Actuator (SES):** Commercial **SES Steer-by-Wire unit** mounted on the single front steering column (`0x169 VCU_SES_REQ` @ 50 Hz). Position encoded with $30000$ raw offset ($0.1^\circ/\text{LSB}$). Dynamic slew rate limits: $125^\circ/\text{s} \to 525^\circ/\text{s}$.
