...
// Valid 0x721 status: aligned, stroke=600 (0mm)
...
static void bootstrap_active(sys::BrakeControl& bc) {
    ...
    can::VcuSebReq dummy;
    bc.tick(false, false, 0, can::Mode::Manual, seb_ok, dummy);
}
...
int main() {
    ...
    printf("-- Priority: ESTOP > lever > CAN pressure > release --\n");
    {
        ...
        bc.tick(false, true, 5000, Mode::Estop, seb_ok, out);
        ...
        bc.tick(true, false, 2000, Mode::Auto, seb_ok, out);
        ...
        bc.tick(false, false, 2000, Mode::Auto, seb_ok, out);
        ...
        bc.tick(false, false, 0, Mode::Manual, seb_ok, out);
        CHECK(out.control_mode == 0);     // Stroke Mode
        CHECK(out.stroke_req == 600);     // 0mm
        CHECK(out.auto_brake == 0);

        // Test 5: lever in MANUAL mode → 15mm
        bc.tick(true, false, 0, Mode::Manual, seb_ok, out);
    ...
    printf("-- auto_brake bit --\n");
    {
        ...
        bc.tick(false, false, 4000, Mode::Auto, seb_ok, out);
        ...
        // ESTOP → auto_brake = 0
        bc.tick(false, true, 0, Mode::Estop, seb_ok, out);
        CHECK(out.auto_brake == 0);
    }
    ...
    printf("-- kPa -> SEB pressure raw --\n");
    {
        ...
        bc.tick(false, false, 5000, Mode::Auto, seb_ok, out);
        CHECK(out.pressure_req == 100);

        // 2500 kPa → (2500+25)/50 = 50
        bc.tick(false, false, 2500, Mode::Auto, seb_ok, out);
        CHECK(out.pressure_req == 50);

        // 100 kPa → (100+25)/50 = 2
        bc.tick(false, false, 100, Mode::Auto, seb_ok, out);
        CHECK(out.pressure_req == 2);

        // 25000 kPa absurd → clamped to 100
        bc.tick(false, false, 25000, Mode::Auto, seb_ok, out);
        CHECK(out.pressure_req == 100);
    }
