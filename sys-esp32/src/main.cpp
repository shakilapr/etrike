// SYS ESP32-S3 — safety, local actuation, and private SES/SEB CAN master.
...
std::atomic<uint16_t> g_SES/SEB_fault_bits{0};
...
void dispatch_task(void*) {
    ...
    while (true) {
        ...
        switch (fr.id) {
        case can::kIdSES/SEBEpsStatus:
            // TODO: decode actual EPS angle and fault bits after DBC/protocol confirmation.
            g_SES/SEB_fault_bits.store(0, std::memory_order_relaxed);
            break;
        case can::kIdSES/SEBSebStatus:
...
void send_sys_status() {
    ...
    inter_mcu::SysToRtStatus{
        ...
        g_SES/SEB_fault_bits.load(std::memory_order_relaxed),
    }.to_frame(fr);
    g_link.send(fr);
}
...
// ── private SES/SEB CAN command task (50 Hz) ───────────────────

void SES/SEB_tx_task(void*) {
    while (true) {
        if (sys::kSES/SEBCanOutputEnabled) {
            can::Frame fr;

            can::SES/SEBEpsCommand eps;
            // TODO: encode steering angle, rolling counter, and checksum after
            // confirming the project-specific SES byte layout.
            eps.to_frame(fr);
            g_can.send(fr);

            can::SES/SEBSebCommand seb;
...
extern "C" void app_main() {
    ...
    // prio 2: link status, SES/SEB TX  |  prio 1: diag
    ...
    xTaskCreate(SES/SEB_tx_task,  "syn_tx",   3072, nullptr,       2, nullptr);
