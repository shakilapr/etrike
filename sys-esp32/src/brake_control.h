#pragma once
// SES/SEB SEB brake control — boot state machine, 50 Hz TX, rolling ctr + checksum.
...
private:
    void build_command(bool lever, bool estop, int32_t brake_kpa, can::VcuSebReq& out) {
        ...
        out.roll_cnt_enable = 1;  // required by SES/SEB spec
        out.checksum_enable = 1;  // required by SES/SEB spec
