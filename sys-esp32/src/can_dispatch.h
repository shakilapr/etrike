...
struct DispatchTargets {
    can::RtDriveCmd* setpoint = nullptr;  // from 0x202
    int32_t* brake_kpa = nullptr;         // from 0x203
...
inline void dispatch_frame(const can::Frame& f, DispatchTargets& t) {
    switch(f.id) {
    case 0x202: if(t.setpoint) *t.setpoint = can::RtDriveCmd::from_frame(f); break;
    case 0x203: if(t.brake_kpa) *t.brake_kpa = can::RtBrakeCmd::from_frame(f).brake_pressure_kpa; break;
