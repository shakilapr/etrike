#!/usr/bin/env python3
"""
Generate sys-esp32/SES/SEB_seb.dbc using canmatrix.

Covers all SES/SEB SEB (electro-hydraulic brake) CAN messages.
...
Usage:
  python generate_seb_dbc.py           # writes SES/SEB_seb.dbc
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "SES/SEB_seb.dbc")

LE = True   # Intel (little-endian) — all SES/SEB frames


def build_database() -> CanMatrix:
    """Construct the SES/SEB SEB CAN database."""

    db = CanMatrix()

    db.add_ecu(Ecu("SYS", "ESP32-S3, FreeRTOS — drives SEB via 0x7B9 at 50 Hz"))
    db.add_ecu(Ecu("SEB", "SES/SEB SEB, electro-hydraulic brake module, preprogrammed CAN IDs"))
    ...
    db.add_frame(msg("VCU_SEB_REQ", 0x7B9, 8, "SYS", cycle_ms=20,
        ...
        comment="SES/SEB SEB brake command. 50 Hz continuous. "
    ...
    db.add_frame(msg("SEB_STATUS", 0x721, 8, "SEB", cycle_ms=10,
        ...
        comment="SES/SEB SEB status feedback. 100 Hz. "
    ...
    db.add_frame(msg("SEB_ErrInfo", 0x731, 8, "SEB", cycle_ms=100,
        ...
        comment="SES/SEB SEB detailed fault flags. 14 of 23 faults are L3 -> "
    ...
    db.add_frame(msg("SEB_Version", 0x741, 8, "SEB", cycle_ms=1000,
        ...
        comment="SES/SEB SEB firmware version. Log on boot for compatibility check.",
    ))
    ...
    db.add_frame(msg("SEB_Test", 0x6FB, 8, "SEB", cycle_ms=10,
        ...
        comment="SES/SEB SEB telemetry. 100 Hz. Bytes 0,7 reserved. "
