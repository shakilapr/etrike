...
static std::atomic<uint8_t>  g_ses_error_status{0};     // 0x201 byte0 bits 6-7: SES error level
...
static void process_frame(const can::Frame& fr, bool from_high, DispatchContext& ctx) {
    ...
    if (fr.id == can::kIdSES/SEBEpsStatus) {
        g_ses_angle_raw.store(ctx.steer_feedback_angle - rt::kSES/SEBAngleOffset);  // CAN raw → 0.1° offset-free
    ...
    if (fr.id == can::kIdSES/SEBEpsErrInfo && !from_high) {
    ...
    if (fr.id == can::kIdSES/SEBEpsVersion && !from_high) {
    ...
    if (fr.id == can::kIdSES/SEBEpsTest && !from_high) {
    ...
    if (fr.id == can::kIdSES/SEBSebTest && !from_high) {
        int16_t  mc_raw = int16_t((uint16_t(fr.data[2]) << 8) | fr.data[1]);
        uint16_t et_raw = (uint16_t(fr.data[4]) << 8) | fr.data[3];
    ...
    if (fr.id == can::kIdSES/SEBSebStatus && !from_high) {
...
[[noreturn]] static void t_control(void*) {
    ...
    while (1) {
        ...
        if (sr.zero_setpoints) {
            ...
            // Send on both buses — SYS, SES, SEB, Host all listen for 0x001
