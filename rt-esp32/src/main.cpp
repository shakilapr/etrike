...
static void process_frame(const can::Frame& fr, bool is_high, DispatchContext& ctx) {
    ...
    if (fr.id == can::kIdSES/SEBEpsErrInfo && !is_high) {
    ...
    if (fr.id == can::kIdSES/SEBEpsVersion && !is_high) {
    ...
    if (fr.id == can::kIdSES/SEBEpsTest && !is_high) {
    ...
    if (fr.id == can::kIdSES/SEBSebTest && !is_high) {
        int16_t  mc_raw = int16_t((uint16_t(fr.data[2]) << 8) | fr.data[1]);
        uint16_t et_raw = (uint16_t(fr.data[4]) << 8) | fr.data[3];
    ...
    if (fr.id == can::kIdSES/SEBSebStatus && !is_high) {
