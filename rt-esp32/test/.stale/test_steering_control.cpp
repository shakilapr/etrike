...
int main(){
    ...
    printf("-- Test 1: Obstacle ESTOP hold angle clamp --\n");
    {
        ...
        int16_t cmd_offset_free = cmd_angle - rt::kSbwAngleOffset;  // strip steer-by-wire offset
    ...
    printf("-- Test 3: Non-obstacle ESTOP ramp-to-zero --\n");
    {
        ...
        int16_t first_step_offset_free = first_step - rt::kSbwAngleOffset;  // strip steer-by-wire offset
    ...
    printf("-- Test 7: exit_estop() deferred (Gap #6) --\n");
    {
        ...
        can::VcuSesReq dummy;
        for (int i = 0; i < 160; ++i) {
            int16_t ses_angle = 0;  // SES tracking perfectly
