...
int main() {
    ...
    // SES/SEB command IDs live only on the SYS private CAN bus.
    {
        can::Frame f;
        can::SES/SEBEpsCommand{}.to_frame(f);
        CHECK(f.id == can::kIdSES/SEBEpsCommand);
        CHECK(f.dlc == 8);

        can::SES/SEBSebCommand{}.to_frame(f);
        CHECK(f.id == can::kIdSES/SEBSebCommand);
        CHECK(f.dlc == 8);
        printf("  ok  SES/SEB private CAN command IDs\n");
    }
    ...
    // All CAN IDs in the catalog are unique.
    {
        uint32_t ids[] = {
            ...
            can::kIdHeartbeat,
            can::kIdSES/SEBEpsCommand, can::kIdSES/SEBEpsStatus,
            can::kIdSES/SEBSebCommand, can::kIdSES/SEBSebStatus,
        };
