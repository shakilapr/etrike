#!/usr/bin/env python3
"""
Generate rt-esp32/SES/SEB_eps.dbc using canmatrix.

Covers all SES/SEB SES (steering-by-wire) CAN messages.
All frames use Intel (little-endian) format per SES/SEB protocol.
Preprogrammed CAN IDs — not reconfigurable.

  Command: 0x169 at 50 Hz continuous (RT -> SES)
  Status:  0x201 at 100 Hz (SES -> RT)
  Errors:  0x202 at 10 Hz (SES -> RT)
  Version: 0x203 at 1 Hz (SES -> RT)
  Test:    0x6FA at 100 Hz (SES -> RT)
...
the SES may report them in alternate frames. Both are included in the DBC
for documentation; tools may warn about the overlap.

Usage:
  python generate_eps_dbc.py           # writes SES/SEB_eps.dbc
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "SES/SEB_eps.dbc")

LE = True   # Intel (little-endian) — all SES/SEB frames


def build_database() -> CanMatrix:
    """Construct the SES/SEB SES CAN database."""

    db = CanMatrix()

    db.add_ecu(Ecu("RT",    "ESP32-S3, FreeRTOS — drives SES via 0x169 at 50 Hz"))
    db.add_ecu(Ecu("SES", "SES/SEB SES, steer-by-wire module, preprogrammed CAN IDs"))
    ...
    # ── 0x169 — VCU_SES_REQ (RT -> SES, 50 Hz continuous) ────────────
    db.add_frame(msg("VCU_SES_REQ", 0x169, 8, "RT", cycle_ms=20,
        signals=[
            sig("SES_AlignEnable",    0,  1, vmin=0, vmax=1, receivers=["SES"],
                comment="Angle Initial Alignment Enable. 0=disabled, 1=centering."),
            sig("SES_CtrlEnable",     1,  1, vmin=0, vmax=1, receivers=["SES"],
    ...
    db.add_frame(msg("SES_STATUS", 0x201, 8, "SES", cycle_ms=10,
        ...
        comment="SES/SEB SES status feedback. 100 Hz. "
                "Byte 5 overlap: StrAngleSpd[15:8] / Torq share byte 5 per CSV.",
    ))

    # ── 0x202 — SES_ErrInfo (SES -> RT, 10 Hz) ────────────────────────
    db.add_frame(msg("SES_ErrInfo", 0x202, 8, "SES", cycle_ms=100,
        signals=[
        ...
        comment="SES/SEB SES detailed fault flags. 8 L3 faults (redundant sensor loss) "
                "-> RT must escalate to ESTOP. Steering CAN comm is L1 (minor).",
    ))

    # ── 0x203 — SES_Version (SES -> RT, 1 Hz) ─────────────────────────
    db.add_frame(msg("SES_Version", 0x203, 8, "SES", cycle_ms=1000,
        signals=[
        ...
        comment="SES/SEB SES firmware version. Log on boot for compatibility check.",
    ))

    # ── 0x6FA — SES_Test (SES -> RT, 100 Hz telemetry) ────────────────
    db.add_frame(msg("SES_Test", 0x6FA, 8, "SES", cycle_ms=10,
        signals=[
        ...
        comment="SES/SEB SES telemetry. 100 Hz. Bytes 0,7 reserved. "
