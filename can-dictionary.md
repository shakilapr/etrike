...
Two physical CAN buses at 500 kbit/s. All fields big-endian (MSB first) unless noted (SES/SEB protocol uses Motorola LSB).
...
Nodes: RT ESP32-S3, SYS ESP32-S3, SES/SEB SES (steering), SES/SEB SEB (brake), DC-DC converter.
...
> Placed at `0x204` to avoid collision with SES error info frame at `0x202`. SES/SEB units are preprogrammed and cannot be reconfigured.
...
RT max-select: `brake_kpa = max(rt_obstacle, jetson_0x301)`. SYS converts: `seb_raw = (uint8_t)(kpa * 0.02f)` (verified SES/SEB spec: `VCU_SEB_Pre_Value_Req` is u8, scale 0.05 MPa/bit, range 0–5 MPa). When `0x205 > 0`, SYS switches SEB to Pressure Mode (mode=2). When `0x205 == 0`, falls back to Stroke Mode for lever/ESTOP triggers.
...
### 0x201 — SES_STATUS (SES/SEB SES Feedback)

| Property | Value |
|----------|-------|
| **Sender** | SES/SEB SES (steering module) |
| **Receiver(s)** | RT |
| **DLC** | 8 |
| **Period** | 10 ms (100 Hz) |
...
### 0x169 — VCU_SES_REQ (SES/SEB SES Command)

| Property | Value |
|----------|-------|
| **Sender** | RT ESP32-S3 |
| **Receiver(s)** | SES/SEB SES (steering module) |
| **DLC** | 8 |
| **Period** | 20 ms (50 Hz) — **continuous, every frame** |
| **Endianness** | Motorola LSB (little-endian) |
| **Note** | Factory default `0x169`. SES/SEB unit is preprogrammed and not reconfigurable. `RT_DRIVE_CMD` placed at `0x204` to avoid collision. |
...
**Security**: If `roll_cnt_enable=0` or `checksum_enable=0`, unit may reject frames. Both must be 1. Checksum algorithm: `XOR(bytes[0..6]) ^ 0xFF` (verify exact formula against SES/SEB spec).
...
### 0x202 — SES_ErrInfo (SES/SEB SES Error Detail)
...
### 0x203 — SES_Version (SES/SEB SES Firmware Version)
...
### 0x6FA — SES_Test (SES/SEB SES Telemetry)
...
### 0x7B9 — VCU_SEB_REQ (SES/SEB SEB Brake Command)

| Property | Value |
|----------|-------|
| **Sender** | SYS ESP32-S3 |
| **Receiver(s)** | SES/SEB SEB (brake module) |
| **DLC** | 8 |
| **Period** | 20 ms (50 Hz) — **continuous, every frame** |
| **Endianness** | Motorola LSB (little-endian) |
...
**Security**: Rolling counter must increment 0→15 every frame. Same value twice → SEB rejects (assumes frozen controller). Checksum = `XOR(bytes[0..6]) ^ 0xFF` (verify against SES/SEB spec).
...
### 0x721 — SEB_STATUS (SES/SEB SEB Brake Feedback)

| Property | Value |
|----------|-------|
| **Sender** | SES/SEB SEB (brake module) |
...
### 0x731 — SEB_ErrInfo (SES/SEB SEB Error Detail)
...
### 0x741 — SEB_Version (SES/SEB SEB Firmware Version)
...
### 0x6FB — SEB_Test (SES/SEB SEB Telemetry)
...
| `0x169` | VCU_SES_REQ | RT | SES | 8 | **50 Hz** |
| `0x201` | SES_STATUS | SES | RT | 8 | 100 Hz |
| `0x202` | SES_ErrInfo | SES | RT | 8 | 10 Hz |
| `0x203` | SES_Version | SES | RT | 8 | 1 Hz |
...
| Low only | `0x201` (SES/SEB SES feedback) |
...
> Lower CAN ID = higher bus arbitration priority. Safety-critical frames occupy `0x00X`. SES/SEB IDs (`0x2XX`, `0x7XX`) are in medium/lowest ranges per manufacturer assignment.
