...
> Placed at `0x202` to avoid collision with SES factory command at `0x200`. SES/SEB units are preprogrammed and cannot be reconfigured.
...
RT max-select: `brake_kpa = max(rt_obstacle, jetson_0x301)`. SYS maps kPa to SEB Pressure Mode via placeholder conversion (`seb_raw = kpa * kSebPressureScale`, TBD: verify scale against SES/SEB spec). When `0x203 > 0`, SYS switches SEB to Pressure Mode (mode=2). When `0x203 == 0`, falls back to Stroke Mode for lever/ESTOP triggers.
...
### 0x200 — VCU_SES_REQ (SES/SEB SES Command)
...
| **Endianness** | Motorola LSB (little-endian) |
| **Note** | Factory default `0x200`. SES/SEB unit is preprogrammed and not reconfigurable. `RT_DRIVE_CMD` placed at `0x202` to avoid collision. |
...
### 0x720 — VCU_SEB_REQ (SES/SEB SEB Brake Command)
...
| `0x200` | VCU_SES_REQ | RT | SES | 8 | **50 Hz** |
