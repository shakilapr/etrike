#!/usr/bin/env python3
"""
Build E-Trike Vero Board Wiring PDF from hand-written LaTeX.

No pandoc needed — vero-board-wiring.tex is standalone LaTeX.
Runs pdflatex once (no TOC needed).

Usage:
  python build-vero-board.py                  # compile vero board wiring PDF
  python build-vero-board.py --date 2026-07-01  # override date
"""
...
VERO_TEX = TEX_DIR / "vero-board-wiring.tex"
...
def compile_pdf(tex_path: Path, output_dir: Path, pdflatex: str) -> Path:
    """Run pdflatex once on a .tex file.
...
def build(args: argparse.Namespace) -> None:
    ...
    print("=" * 72)
    print(f"  E-Trike Vero Board Wiring Builder")
    ...
    dest_tex = GENERATED_DIR / VERO_TEX.name
    if dest_tex.exists():
        dest_tex.unlink()
    content = VERO_TEX.read_text(encoding="utf-8")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Build E-Trike Vero Board Wiring PDF from hand-written LaTeX."
    )
    parser.add_argument(
        "--date", default=today,
