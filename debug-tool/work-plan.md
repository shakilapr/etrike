...
### Third-Party ECUs (SES, SEB): CAN-Level Behavioral Models
...
## 6. Phase 2b — Third-Party ECU Models (SES, SEB): CAN-Level Behavioral Models
...
| # | Item | Depends on | Effort |
|---|------|-----------|--------|
| 2b.1 | Record real SES CAN traffic — connect to CANalyst-II, run steering sweep scenarios (0°→±90°→0°), capture all frames | Hardware access | Small |
...
| 2b.4 | Enhance SES model — add rate-limited angle tracking (1st-order lag), per-fault-bit injection, torque feedback from steering load | 2a.1 | Medium |
...
| 2c.5 | Run same tests with IPC native models (RT/SYS/MTR) + TypeScript models (SES/SEB/HOST) in same engine | 2a.7, 2b.7, 2c.1 | Medium |

**Verification:** Full Simulation mode: keyboard → HOST model → RT model (IPC) → MTR model (IPC) → physics updates → SES model reports angle. All 6 ECUs active. Scenario replay produces identical results across IPC and TypeScript backends.
...
| 3.6 | **Bench Test mode:** Real CAN bus, no emulation. Bypass flags exposed in UI. Per-ECU "what's blocked" diagnostic (e.g., "Steering stuck in LISTEN_SYNC — SES absent"). | 3.2 | Medium |
...
**Verification:** Each mode starts with one click. Hybrid mode: unplug SES → auto-starts SES emulation → steering pipeline works. Bench mode: shows "SES absent — steering blocked at LISTEN_SYNC."
...
| 4.6 | "Missing ECU" diagnostic panel — shows dependency chain and what's blocked (e.g., "RT needs SES 0x201 for steering sync → currently absent → bypass available") | 3.4 | Medium |
...
| ECU models — SES, SEB (TypeScript) | 2b.4-2b.7 | ⚠️ In `simulation/src/ecus/` — needs validation against hardware |
| Hardware captures (SES, SEB) | 2b.1-2b.2 | ❌ Not recorded yet — needs CANalyst-II + real units |
...
Phase 0 (foundation fixes)
  └─→ Phase 1 (frame pipeline, transport)
        ...
        └─→ Phase 2b (SES/SEB CAN-level models)    ──┤
...
| No access to real SES/SEB hardware for captures | Rely on manufacturer CSV protocol documents as specification; TypeScript models implement the documented contract; validate later when hardware available |
