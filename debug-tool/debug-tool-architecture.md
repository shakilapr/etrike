...
Since we do not have access to vendor source code, third-party actuators (SES, SEB) are the *only* units mimicked in TypeScript. They are implemented as simple state machines within `backend/src/sim/ecus/` and rely on `encodePayload()` to serialize data dynamically.
