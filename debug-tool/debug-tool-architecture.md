...
| `0x201` | SES_STATUS | SES | 8 | angle_status, str_angle (i16 LE), torque, error_status |
| `0x202` | SES_ErrInfo | SES | 8 | 25 fault flags (u32 LE mask) |
| `0x203` | SES_Version | SES | 8 | sw_version, hw_version |
...
| `0x6FA` | SES_Test | SES | 8 | motor_current, ecu_temp, supply_voltage |
...
debug-tool/
  ...
  sim/                               ← NEW in v0.4.0-alpha
    ecus/
      ...
      epsc.ts                        SES steering actuator model
...
| **SES** | Steering actuator | Angle tracking, Alignment detection, 25 fault bits, L1/L2/L3 error levels | 0x169 | 0x201, 0x202, 0x203, 0x6FA |
...
```
Source Table (example — Hybrid mode with SES absent):
  ...
  low:0x201  → emulated     (SES emulated — no real SES detected)
  low:0x202  → emulated     (SES fault status)
...
interface WorkModeConfig {
  ...
  bypasses: {
    epscSync: boolean;    // Skip SES steering sync → RT can drive without SES
...
**Bypass flags at runtime:** Currently bypass flags are compile-time only (`-D CONFIG_BYPASS_SES_SYNC` in platformio.ini). The architecture supports runtime bypass via the ECU model's `config()` call — the model simply skips the listen-sync requirement when the flag is set. For real hardware, bypass flags remain compile-time until a runtime config protocol is added to the firmware.
...
**Physics model** (`src/sim/physics/`): Tricycle kinematics — converts drive commands (speed, yaw) into per-wheel speeds, steering angle, and brake force. Feeds back into actuator models (SES angle, MTR load, SEB pressure).
...
| | Our ECUs (RT, SYS, MTR) | Third-party (SES, SEB) |
|---|---|---|
| **Have source code?** | ✅ C++ in `rt-esp32/src/`, `sys-esp32/src/`, `mtr-stm32/src/` | ❌ Vendor black boxes |
| **Platform** | ESP32-S3 (RT, SYS), STM32F103C8 (MTR) | SES, SEB — proprietary controllers |
...
#### 14.8.2 Third-Party ECUs (SES, SEB): CAN-Level Behavioral Models
...
1. **Subscribes to command frames** — e.g., SES listens for `0x169 VCU_SES_REQ`, SEB listens for `0x7B9 VCU_SEB_REQ`, MTR listens for `0x204 RT_DRIVE_CMD`
2. **Runs a simple internal model** — angle tracking with rate limiting (SES), stroke/pressure response with first-order lag (SEB), speed tracking (MTR)
...
6. **Accepts physics plant input** — `setActualAngle(deg)` for SES, `setActualStroke(mm)` for SEB, `setActualSpeed(mmps)` for MTR — the physics model drives the actuator model, which reports realistic feedback

**Validation strategy for third-party models:**

```
1. Record:    Connect real SES/SEB/MTR to CANalyst-II, run test scenarios, capture all frames
...
|-----|---------------|------------|
| SES | 0x201, 0x202, 0x203, 0x6FA | Angle tracking is 0th-order (instant, no rate limit). Fault injection is binary (L3 on/off), doesn't exercise individual fault bits. No torque feedback model. |
...
| ECU model — SES | `backend/src/sim/ecus/epsc-model.ts` | ✅ Angle tracking, 25 fault bits, checksums, L1/L2/L3 |
...
- Capture→replay validation for SES/SEB models against real hardware
