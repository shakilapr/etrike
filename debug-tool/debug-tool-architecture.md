...
A CAN bus diagnostic, monitoring, and hardware-in-the-loop test tool for the E-Trike vehicle control system. Connects to one or both CAN buses via an ESP32-S3 bridge over MQTT/Wi-Fi, streams decoded frames to a web UI, and can inject commands to simulate any node — Jetson, RT, SYS, MTR, or SES/SEB actuators.
...
```
Inject 0x300 (high) → watch RT produce 0x204 + 0x169 (low) → see SES respond 0x201 (low)
```
...
| `0x201` | SES_STATUS | SES | 100 Hz | 8 | `angle_status`, `str_angle`, `tgt_angle_spd`, `error_status` | ✅ |
| `0x202` | SES_ErrInfo | SES | 10 Hz | 8 | 25 fault flags (8× L3) | — |
| `0x203` | SES_Version | SES | 1 Hz | 8 | SW + HW version | — |
...
| `0x6FA` | SES_Test | SES | 100 Hz | 8 | motor current, ECU temp, supply voltage | — |
...
void decode_frame(const can::Frame& fr, bool is_low_bus, char* buf, size_t max_len) {
    ...
    switch (fr.id) {
        ...
        case can::kIdVcuSesReq: {
            ...
            snprintf(buf, max_len, /* SES/SEB little-endian fields */);
            break;
        }
