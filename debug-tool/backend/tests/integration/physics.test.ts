...
describe("SIL Physics & Dynamics", () => {
  ...
  it("translates yaw rate into steering angle target dynamically", async () => {
    ...
    const sesReq = hubEvents.filter(e => e.payload.id === "0x169").pop();
    expect(sesReq).toBeDefined();
    // 1000 mrad/s * 0.05 = 50
    expect(sesReq.payload.decoded.target_angle).toBe(50);
  });
  ...
  it("brakes sharply to 0 when ESTOP is triggered during motion", async () => {
    ...
    expect(mtrFb.payload.decoded.actual_speed_mmps).toBeGreaterThan(1990);
