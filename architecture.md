...
Three physical CAN buses: two at 500 kbit/s (high-level and low-level) and one at 250 kbit/s (powertrain). RT bridges selected messages between high and low buses. PWT bridges selected messages between low and powertrain buses. Actuators are **SES/SEB** CAN modules: SES (steer-by-wire) and SEB (electro-hydraulic brake). **Mode-gated dual control (Option D):** RT directly commands both SES and SEB in AUTO mode; SYS commands SEB in MANUAL mode (lever pass-through) and on ESTOP. This ensures no single MCU failure takes both actuators, and AUTO-mode brake+steer are 1-hop from the kinematics engine. Motor control is on a dedicated STM32 board (MTR) for safety isolation per ISO 26262 EGAS 3-level concept.
...
| `0x169` | VCU_SES_REQ | RT | SES (steering) | 8 | Angle cmd + security bytes | 50 Hz | Medium |
| `0x201` | SES_STATUS | SES | RT | 8 | Steering angle + status feedback | 100 Hz | Medium |
| `0x202` | SES_ErrInfo | SES | RT | 8 | 25 fault flags (8× L3) | 10 Hz | Medium |
| `0x203` | SES_Version | SES | RT | 8 | SW + HW version | 1 Hz | Lowest |
...
| `0x6FA` | SES_Test | SES | RT | 8 | Motor current, ECU temp, supply voltage | 100 Hz | Lowest |
...
> **ID note**: SES/SEB units are preprogrammed and cannot be reconfigured. SES uses factory command `0x169` and status `0x201`, plus diagnostic frames `0x202` (err info), `0x203` (version), `0x6FA` (telemetry). SEB uses factory command `0x7B9` and status `0x721`, plus diagnostic frames `0x731` (err info), `0x741` (version), `0x6FB` (telemetry). `RT_DRIVE_CMD` is placed at `0x204` to avoid collision with SES `0x169`. `0x205` RT_BRAKE_CMD avoids collision with `0x202` and `0x203`.
>
...
| Low only | `0x201`, `0x202`, `0x203`, `0x6FA` | SES status and diagnostics — RT consumes locally for boot sync, monitoring, and fault handling |
...
| Mode | Behavior |
|------|----------|
| **MANUAL** | Rider steers / rides throttle. **MTR** reads throttle ADC + gear sense → pass-through via MCP4725 + relays. Brake lever → SYS GPIO → CAN `0x7B9` → SEB. SES standalone (RT idle). DC-DC on. Mode gated by SYS `0x110`. |
| **AUTO** | Jetson `AckermannControlCommand` → high CAN `0x300` → RT kinematics → low CAN `0x204` (MTR: speed+gear) + `0x169` (SES: angle). **MTR** drives MCP4725 + gear relays following `0x204`. Lights from Jetson via `0x302` (RT fwd). Brake via `0x7B9`. Mode gated by SYS `0x110`. |
...
Steering wheel        ──► SES standalone (RT idle, monitors 0x201)
...
Jetson AckermannControlCommand ──► High CAN 0x300 ──► RT kinematics
               ...
               ┌──────────────────────────┤
               ▼ (low CAN)                ▼ (low CAN)
   0x204 {speed, gear} → MTR       0x169 {angle} → SES
               │                          │
...
**What the Jetson controls:** Nothing directly. All actuation is via CAN commands to RT (which forwards to SYS/MTR and commands SES). The Jetson is a pure perception/planning/command node — no GPIO, no analog I/O, no direct actuator wiring.
...
5. **All multi-byte CAN fields are big-endian (MSB first)** — unless SES/SEB protocol specifies otherwise (see `0x169`, `0x7B9` in can-dictionary).
6. **Manual mode is pass-through, not dead.** SYS mirrors physical inputs to outputs.
7. **Actuators are standalone CAN modules.** SES, SEB, and DC-DC are commanded via CAN.
8. **RT is the only dual-bus node.** No direct Jetson ↔ SYS path.
9. **Listen Before Speaking.** SES/SEB units require receiving status feedback before any command is sent. Boot state machines enforce this.
...
11. **Mode-gated dual control of SES/SEB actuators (Option D).** In AUTO, RT commands both SES and SEB directly — 1-hop from the kinematics engine. In MANUAL, SYS commands SEB based on lever position; SES runs standalone. The `0x7B9` SEB command is dual-sender but mode-gated — only one node transmits at a time, no collision. Per ISO 26262-5:2018 §7.4.4, this is redundant-controller practice. No single MCU failure can take both actuators offline.
...
| **Why only motor needed MTR** | SES/SEB SES and SEB already do EGAS Level 1 internally (CAN, PID, angle sensor, fault detection). Motor controller is a dumb analog device — it has no intelligence, no CAN, no internal monitoring. That gap is filled by the dedicated MTR board. |
...
| SES (steering) | Internal boot delay ~2s, then enters centering | Does not accept commands until internally aligned |
...
actively brake. The SES centering sequence may briefly move the steering
...
### 6.2 Mode-Gated Dual Control (Option D) — SES/SEB Actuators

The SES and SEB are commanded by different nodes depending on mode. Four options were evaluated (see §6.3). Option D was selected:
...
**Why Option D over C (SYS owns both):** C makes SYS a single point of failure for both actuators. D preserves independent failure modes — RT failure loses AUTO steering/brake but SYS can still brake in MANUAL; SYS failure loses MANUAL brake but RT can still brake in AUTO. The SES's internal timeout (20ms comm loss → lock) and SEB's internal timeout provide hardware backup regardless.

**Why not B (private CAN):** Adds hardware cost (MCP2515 + transceiver + bus wiring) and SYS bridging latency without safety improvement. The shared low-level CAN with SES/SEB's own rolling counter + checksum validation already prevents accidental actuation.

| Property | RT (AUTO) | SYS (MANUAL/ESTOP) |
|----------|-----------|---------------------|
| SES (0x169) | Sends angle from kinematics | Does NOT send — SES standalone |
| SEB (0x7B9) | Sends arbitrated brake kPa→pressure | Sends lever stroke / ESTOP max |
| 0x201 (SES status) | Monitors angle for following error | Does not monitor |
...
| RT failure → steer lost? | Yes | Same | No (SYS bridges) | Yes (SES timeout backup) |
...
- **Steering angle** → low CAN `0x169` → SES/SEB SES
...
struct ResolvedSetpoint {
    ...
    int32_t steer_angle_mdeg = 0;  // ±45000, +right (internal; convert to decideg for SES/SEB)
    uint8_t gear             = 0;
...
#### Steering — SES/SEB SES via CAN (`0x169`)
...
STEER_FAULT:
  ...
  - SES will timeout-fault internally
...
**Unit conversion** (internal mdeg ↔ SES/SEB decideg):

```
SES/SEB raw = internal_angle_mdeg / 100   (45500 mdeg → 455 raw → 45.5°)
internal_mdeg = SES/SEB raw * 100         (455 raw → 45500 mdeg)
```

**SES/SEB protocol specifics:**

| Parameter | Value |
|-----------|-------|
| Command ID | `0x169` (factory default — SES/SEB preprogrammed, not reconfigurable. `0x7B9` is sent by RT in AUTO, SYS in MANUAL/ESTOP — one sender active at a time, no collision. `0x205` removed (RT now sends SEB directly)) |
...
| **Timeout fault** | If `0x169` stops for >20 ms, SES triggers internal comm fault. RT must maintain 50 Hz in AUTO. |
...
|------|------------------|
| MANUAL | RT does NOT send `0x169`. SES standalone. RT still listens `0x201` for telemetry. |
...
constexpr int   kSteerCmdRateHz = 50;              // SES/SEB requires 20ms period
...
//   kIdRtBrakeCmd (0x205), kIdSES/SEBEpsCmd (0x169), kIdSES/SEBSebCmd (0x7B9), etc.
...
| Steering CAN TX fail | TWAI TX errors | Log, SES will timeout-fault |
...
| `0x201` SES_STATUS — DLC=8, `{u8 status, i16 angle, u8 torq, …}` | Steering SM: boot sync angle, alignment check, following-error monitor, SES/SEB rolling counter + XOR checksum | `0x169` VCU_SES_REQ — DLC=8, `{u8 ctrl, i16 angle, u8 speed, u8 sec, u8 cnt+cksum, u8 cksum}` → SES (50 Hz, Angle Mode) | Steering angle |
...
**What RT controls:** SES steering angle via `0x169` (Angle Mode, 50 Hz, SES/SEB rolling counter + checksum security), MTR motor speed + gear via `0x204` (100 Hz), SEB brake via `0x205`→SYS→`0x7B9` (50 Hz, Pressure/Stroke Mode). CAN gateway message forwarding between low and high buses.
...
| **Brake control** | Lever sensor (GPIO) → CAN 0x7B9 → SES/SEB SEB. ESTOP → max stroke (27mm). MANUAL → lever stroke (15mm). AUTO → converts RT 0x205 kPa to SEB pressure mode. **Brake priority: ESTOP > lever > CAN pressure > released.** | Prio 3 |
...
| `0x7B9` | VCU_SEB_REQ | `{u8 ctrl[2], u16 stroke, u16 press, u8 sec, u8 cksum}` (8 bytes) | **50 Hz** | → SES/SEB SEB |
...
#### Brake — SES/SEB SEB via CAN (`0x7B9`)
...
> 
> RT sends `0x205 {i32 brake_pressure_kpa}`. SYS converts using verified SES/SEB SEB spec:
...
```
 Low-Level CAN (500 kbit/s)
  ...
  ├── SES/SEB SES (steering)   TX: 0x201 | RX: 0x169
  ├── SES/SEB SEB (brake)        TX: 0x721 | RX: 0x7B9
...
| 3 | ~~SES timeout-fault behavior unknown~~ | ~~On ESTOP or comm loss, steering may lock, center, or freewheel~~ | **RESOLVED:** Two-tier ESTOP steering (§7.6). Active-zero centering for non-obstacle ESTOP with silent-stop fallback on mechanical jam. Obstacle-triggered ESTOP holds current angle. SES timeout-fault is now a last-resort fallback (CAN bus dead), not the primary ESTOP mechanism. |
| 4 | ~~SEB pressure control mode not defined~~ | ~~SYS currently uses stroke mode only~~ | **RESOLVED:** Verified SES/SEB SEB spec: `VCU_SEB_Pre_Value_Req` is u8 at bit 32, scale 0.05 MPa/bit, range 0–5 MPa (raw 0–100). Conversion: `seb_raw = kPa × 0.02`. Clamp to 100. |
...
| [`docs/steering-unit.md`](docs/steering-unit.md) | SES/SEB SES protocol reference |
| [`docs/brake-unit.md`](docs/brake-unit.md) | SES/SEB SEB protocol reference |
...
| [`docs/SES/SEB-security-protocol.md`](docs/SES/SEB-security-protocol.md) | Rolling counter + XOR checksum for SES/SEB actuators |
