...
> **ID note**: SES/SEB units are preprogrammed and cannot be reconfigured. SES uses factory command `0x169` and status `0x201`. SEB uses factory command `0x7B9` and status `0x721`. `RT_DRIVE_CMD` is placed at `0x204` to avoid collision with SES `0x169`.
