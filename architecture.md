...
| Mode | Behavior |
|------|----------|
| **MANUAL** | Rider steers / rides throttle. SYS reads throttle ADC + gear sense → pass-through via MCP4725 + relays. Brake lever → SYS GPIO → CAN `0x7B9` → SEB. SES standalone (RT idle). DC-DC on. |
| **AUTO** | Jetson `/cmd_vel` → high CAN `0x300` → RT kinematics  → low CAN `0x204` (SYS: speed+gear) + `0x169` (SES: angle). SYS drives MCP4725 + gear relays. Lights from Jetson via `0x302` (RT fwd). Brake via `0x7B9`. |
...
Jetson /cmd_vel ──► High CAN 0x300 ──► RT kinematics 
               ...
               ┌──────────────────────────┤
               ▼ (low CAN)                ▼ (low CAN)
   0x204 {speed, gear} → SYS        0x169 {angle} → SES
               │                          │
