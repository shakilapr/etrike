...
- **Low-level CAN (500 kbit/s):** RT, SYS, PWT, SES (steering), SEB (brake). Actuator commands, safety status, mode control, motor feedback.
...
| Low | `0x201` | SES_STATUS | SES → RT: angle feedback |
...
| Mode | Behavior |
|------|----------|
| **MANUAL** | Rider steers, throttle grip → MTR pass-through → motor. Brake lever → SYS → SEB. SES standalone. DC-DC on. |
...
5. **All multi-byte CAN fields big-endian** unless SES/SEB protocol specifies otherwise.
...
| 0x169 | TX (low) | 100 Hz | Steering angle → SES. Checksum XOR^0xFF. Gated: only in AUTO/ESTOP. |
...
The system has hardware dependencies on peer ECUs and SES/SEB actuators. For
...
| `CONFIG_BYPASS_SES_SYNC` | Skip SES listen-sync. Steering assumes centered (0°). | RT |
...
**Low-level (500 kbit/s):** RT, SYS, PWT, SES, SEB
...
|-----|-------|-----------|-----------|
| RT | esp32-s3-devkitc-1 | espidf | `-D CONFIG_BENCH_SOLO -D CONFIG_BYPASS_SES_SYNC` (bench only) |
...
| 11 | SES/SEB checksum on 0x201 | ✅ Added to RT dispatch. |
