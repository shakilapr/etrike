...
> **ID note**: SES/SEB units are preprogrammed and cannot be reconfigured. SES uses factory command `0x200` and status `0x201`. SEB uses factory command `0x720` and status `0x721`. `RT_DRIVE_SETPOINT` is placed at `0x202` to avoid collision with SES `0x200`.
...
**Not forwarded**: `0x300`, `0x301` (consumed by RT); `0x200`, `0x202` (RT-generated on low); `0x210`, `0x220`, `0x400` (RT-generated on high); `0x012`, `0x110`, `0x720` (low only); `0x201`, `0x721` (low only, SES/SEB feedback); `0x7FF` (per-bus, alive counter, not bridged).
...
STEER_FAULT:
  ...
  - SES will timeout-fault → locks or goes limp (TBD by unit spec)
```
...
| ESTOP | RT stops sending `0x200`. SES timeout-faults (lock/limp TBD). |
...
| 4 | SES timeout-fault behavior unknown | On ESTOP or comm loss, steering may lock, center, or freewheel | Verify with SES/SEB spec; implement appropriate mechanical safety |
...
| [`notes/SES/SEB-security-protocol.md`](notes/SES/SEB-security-protocol.md) | Rolling counter + XOR checksum for SES/SEB actuators |
