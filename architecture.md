...
| **AUTO** | Jetson `/cmd_vel` → high CAN `0x300` → RT kinematics → low CAN `0x204` (MTR: speed+gear) + `0x169` (SES: angle). **MTR** drives MCP4725 + gear relays following `0x204`. Lights from Jetson via `0x302` (RT fwd). Brake via `0x7B9`. Mode gated by SYS `0x110`. |
