  ...
  tick(
    ...
    for (const f of lowBusRx) {
      switch (f.canId) {
        ...
        case "0x201": {
          // SES_STATUS — SES steering feedback
    ...
    if (nowMs % 20 === 0) {
      ...
      // Matches firmware VcuSebReq::pack() — per SES/SEB CSV: strokemode, stroke=1140(max), rolling counter, xor^0xFF checksum
      if (sysHbTimeout) {
    ...
    if (nowMs % (1000 / STEER_CMD_RATE_HZ) === 0) {
      ...
      if (cmd) {
        ...
        // Compute checksum: XOR(bytes 0-6) ^ 0xFF (per SES/SEB CSV spec)
        let cksum = 0;
