...
export class RtSteeringController {
   ...
   * @param sesAngleRaw  SES angle in 0.1° units, or `null` if no data
  ...
  tick(sesAngleRaw: number | null, sesAngleStatus: number, nowMs: number): VcuSesReq | null {
    ...
    switch (this.state) {
      ...
      case SteerState.LISTEN_SYNC: {
        ...
        // Alignment check: SES must report angle_status == 1
        if (sesAngleStatus === 0) return null;
        // Synchronized — convert raw SES/SEB (0.1°+30000 offset) to millidegrees
  ...
  private buildCommand(): VcuSesReq {
    ...
    // Convert millidegrees to SES/SEB raw format:
