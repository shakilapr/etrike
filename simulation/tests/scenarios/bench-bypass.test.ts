 ...
 * (CONFIG_BENCH_SOLO, BYPASS_SES_SYNC, etc.) enable on real hardware.
 */
...
// ═══════════════════════════════════════════════════════════════
//  CONFIG_BYPASS_SES_SYNC — no steering actuator present
// ═══════════════════════════════════════════════════════════════

describe("CONFIG_BYPASS_SES_SYNC — no steering actuator", () => {
  it("RT does not FAULT when SES is absent", () => {
    // Without SES, 0x201 never arrives. Bypass skips LISTEN_SYNC.
    const runner = new SimulationRunner();
    runner.configure(cfg({
      hostDriveCycle: [{ durationMs: 99999, speedMmps: 500, yawRateMradS: 0, gear: 1 }],
    }));
    const result = runner.runDuration(3000);
    // Should not have validation errors from missing SES
  ...
  it("RT still sends 0x169 even without SES feedback", () => {
    const runner = new SimulationRunner();
    runner.configure(cfg({
...
describe("Full bench bypass — all flags active", () => {
  it("RT+SYS on same bus: no peer ECUs, no actuators — stable for 30s", () => {
    ...
    // CANalyst-II as Host on high bus. No SES, SEB, MTR, or PWT.
    const runner = new SimulationRunner();
    runner.configure(cfg({
      hostDriveCycle: [
