 ...
 *
 * Groups: Low-bus custom, High-bus custom, SES/SEB SES, SES/SEB SEB,
...
// ================================================================
//  HIGH BUS — Custom Protocol (IDs ≥ 0x300, except SES/SEB)
// ================================================================
...
// ================================================================
//  SES/SEB SES (Steering) — IDs 0x169, 0x201, 0x202, 0x203, 0x6FA
// ================================================================

describe("0x169 VCU_SES_REQ (RT→SES)", () => {
  it("RT sends at 50 Hz, DLC=8, valid SES/SEB checksum", () => {
    const runner = new SimulationRunner();
...
describe("0x201 SES_STATUS (SES→RT)", () => {
  it("SES sends at 100 Hz with angle, status, checksum", () => {
    const runner = new SimulationRunner();
    runner.configure(drivingCfg(500, 100));
    const result = runner.runDuration(100);
    expect(result.lowBus.total).toBeGreaterThan(3);
...
describe("0x203 SES_Version", () => {
  it("SES sends at 1 Hz with SW/HW version", () => {
    const runner = new SimulationRunner();
    runner.configure(drivingCfg(500));
...
describe("0x6FA SES_Test", () => {
  it("SES sends at 100 Hz (motor current, temp, voltage)", () => {
    const runner = new SimulationRunner();
...
// ================================================================
//  SES/SEB SEB (Brake) — IDs 0x7B9, 0x721, 0x731, 0x741, 0x6FB
// ================================================================

describe("0x7B9 VCU_SEB_REQ (SYS→SEB)", () => {
  it("SYS sends at 50 Hz, DLC=8, valid SES/SEB checksum", () => {
    const runner = new SimulationRunner();
    runner.configure(drivingCfg(500));
    const result = runner.runDuration(100);
    expect(result.lowBus.total).toBeGreaterThan(2);
...
describe("Full drive cycle", () => {
  it("Jetson→RT→SYS/MTR/SES/SEB completes without errors", () => {
    const runner = new SimulationRunner();
    runner.configure(autoCfg({
      jetsonDriveCycle: [
