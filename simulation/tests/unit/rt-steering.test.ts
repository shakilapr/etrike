...
describe("RtSteeringController", () => {
  ...
  it("transitions to ACTIVE on valid 0x201 data", () => {
    ...
    // Valid SES data: angle = 30000 (0°), aligned = 1
    const now = 500;
  ...
  it("stays in LISTEN_SYNC when angle_status is 0 (center-finding)", () => {
    ...
    // SES sends angle but not yet aligned
  ...
  it("setTarget updates active angle in ACTIVE state", () => {
    ...
    // Raw angle = 15000/100 + 30000 = 30150
    sc.setTarget(15000, 2000);

    const out = sc.tick(null, 1, 520);
    expect(out!.targetAngle).toBe(30150);
  });

  // ── ESTOP behavior ───────────────────────────────────────────────

  it("ramps toward zero on non-obstacle ESTOP", () => {
    // Boot + sync + set angle to 20° (20000 millidegrees → raw = 20000/100+30000 = 30200)
    for (let i = 0; i < 26; i++) sc.tick(null, 0, i * 20);
    sc.tick(30000, 1, 500);
    ...
    expect(out!.targetAngle).toBeLessThan(30200);
  ...
  it("holds then silences on obstacle ESTOP", () => {
    ...
    for (let i = 0; i < 26; i++) sc.tick(null, 0, i * 20);
    sc.tick(30000, 1, 500);
    sc.setTarget(10000, 2000); // 10° → raw = 10000/100+30000 = 30100
