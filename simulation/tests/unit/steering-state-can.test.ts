...
describe("RtSteeringController — CAN output per state", () => {
  ...
  // SteerState.LISTEN_SYNC — waiting for SES sync, no 0x169
  it("STEER_LISTEN_SYNC: no 0x169 without valid SES data", () => {
    // Boot complete → LISTEN_SYNC
    for (let i = 0; i < 26; i++) sc.tick(null, 0, i * 20);
    expect(sc.state).toBe(SteerState.LISTEN_SYNC);

    // Still no valid SES data → no output
  ...
  it("STEER_ACTIVE: tick() returns VcuSesReq (0x169 transmitted)", () => {
    // Boot + sync
    for (let i = 0; i < 26; i++) sc.tick(null, 0, i * 20);
    sc.tick(30000, 1, 500);  // valid SES: angle=0°, aligned=1
...
describe("Simulation — CAN output during steering states", () => {
  ...
  it("Sync timeout → FAULT: no 0x169 after fault", () => {
    // We can force FAULT by never providing SES data.
    // RtEcu ticks but never receives 0x201 SES_STATUS.
    // The simulation runs with a default SES that provides data.
    ...
    // The simulation includes SES which syncs, so ACTIVE is reached.
...
describe("State transitions from steering_control.h", () => {
  ...
  it("LISTEN_SYNC → ACTIVE on valid SES data with angle_status=1", () => {
    for (let i = 0; i < 26; i++) sc.tick(null, 0, i * 20);
    expect(sc.state).toBe(SteerState.LISTEN_SYNC);

    // Valid SES: ses_angle_raw=30000 (0°), angle_status=1 (aligned)
  ...
  it("LISTEN_SYNC stays when angle_status=0 (center-finding)", () => {
    ...
    // SES sends angle but not aligned
