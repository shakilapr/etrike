...
- **SYS-authority caveat (verified against real firmware).** `rt-esp32` requires `READY_BIT_SAFETY(0x011) | READY_BIT_MODE(0x110) | READY_BIT_HOST(0x300)` for motion (`rt-esp32/src/safety_stream_loss.h:50-51`). rm additionally emits `SYS_SAFETY_STS` (0x011, `estop_active=0`, valid E2E CRC) and `SYS_MODE_CMD` (0x110, `mode=AUTO` when driving) at 10 Hz. **However, real `rt-esp32` only consumes `0x011`/`0x110` from its LOW bus** (`can_rx_router.h:64-84`, `can_dispatch.h:157,366`) while Host commands are consumed only from its HIGH bus (`can_rx_router.h:28/36/44`). Because rm is a single-bus device, these authority frames land on High and are ignored; `g_bench_solo_mode` does not bypass the motion gate (`rt-esp32/src/main.cpp:1100` is unconditional). **Therefore a standalone rm+rt bench cannot grant motion** — it needs the real `sys-esp32` on rt's Low bus, or an rt bench build that accepts high-bus authority. The system testbench's `RtNode` accepts `0x110` on either bus and omits the `0x011` gate, so it does not reproduce this. See `native-test` target `rm_gateway_ingest`.
