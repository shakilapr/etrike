...
can_high.yaml + can_low.yaml
 ...
 normalized protocol model
   |        |         |          |
firmware  UI/API   golden tests  docs
 codecs   metadata    and traces
```
...
This split is the largest architectural gap. The UI can faithfully decode the YAML while the ECUs faithfully transmit a different hand-written layout.

## 3. CAN contract and generation gaps

| ID | Severity | Gap and evidence | Bench/UI consequence | Required resolution |
|---|---|---|---|---|
...
| CAN-002 | High | Forwarding arrays in [`generate_code.py`](shared/can/generate_code.py), [`candex/examples/etrike/candex.yaml`](candex/examples/etrike/candex.yaml), the copied native forwarding test, and architecture tables are hand-maintained. They omit HMI `0x111`/`0x112`, while [`can_protocol.h`](shared/can/can_protocol.h) and RT routing forward them. | Generated/existing tools and tests describe a different route from firmware. | Put route metadata in canonical YAML and generate runtime routing, Candex/example metadata, documentation, and tests from it. |
...
| CAN-004 | High | Neither YAML nor firmware exposes a protocol semantic version/hash over CAN. | The Control UI cannot prove that a flashed ECU matches the catalog used to decode it. | Generate a protocol hash from normalized YAML, embed it in every firmware image, and expose it through a version/diagnostic frame or diagnostic request. Record it in every test session. |
...
| CAN-008 | Medium | Running `generate_code.py --verify` reported success but rewrote the generated timestamp comments during this audit. | A verification command dirties the working tree, makes reproducibility checks noisy, and can conceal meaningful generated changes. | Make verify byte-for-byte read-only and make generated output deterministic by omitting wall-clock timestamps or deriving stable build metadata separately. |

## 4. Frame-level mismatches

| ID | Severity | Contract/document claim | Actual code | Bench/UI consequence and resolution |
|---|---|---|---|---|
...
| FRM-002 | High | Low YAML defines `0x7FE` byte 1 as heartbeat bit 0, ESTOP bit 1, bits 2–3 reserved. | SYS sets bit 2 `mode_auto` and bit 3 `can_ok`, matching the root architecture rather than YAML. | UI loses two health indicators or reports reserved-bit corruption. Update YAML and generate named bit masks/codecs. |
| FRM-003 | High | Both YAML files define `SYS_DiagHeartbeatOk` as all eight bits of byte 2 and omit RX overflow. | [`can_protocol.h`](shared/can/can_protocol.h) packs heartbeat OK in byte 2 bit 0 and a six-bit, saturating `rx_overflow` in bits 1–6; SYS transmits it. | Every nonzero overflow corrupts the UI's apparent heartbeat value. Correct the YAML to bit 0 plus bits 1–6, reserve bit 7, and add counter semantics. |
| FRM-004 | High | `0x7FC HOST_HEARTBEAT` is DLC 2 in YAML and the shared protocol: alive counter plus health flags. | [`vehicle_bridge_node.cpp`](jetson/src/autoware_vehicle_bridge/src/vehicle_bridge_node.cpp) sends DLC 1. RT currently reads only byte 0. | The system can appear alive while strict validation correctly flags every heartbeat malformed. Generate the host codec and send DLC 2; do not relax UI validation to DLC 1. |
| FRM-005 | High | HMI `0x111` mode values are `0=MANUAL`, `1=AUTO`, `2=PURE_SIM`; the shared global `Mode` uses value 2 for ESTOP. | SYS explicitly accepts only values 0 and 1. | A generated UI may offer `PURE_SIM`, which SYS rejects, and another decoder may label 2 as ESTOP. Use distinct enums for requested operating mode and reported safety/mode state; either implement value 2 or remove it from the wire contract. |
...
| TIM-002 | Medium | YAML marks `SYS_MODE_CMD 0x001` as on-change, while SYS also refreshes it every second. | The UI may report unsolicited traffic or calculate an incorrect timeout. Model `on_change` plus `refresh_ms: 1000`. |
...
| TIM-005 | High | SYS marks the SEB rolling counter unhealthy after one duplicate and healthy after one change. SEB status is faster than the command counter, so duplicates can be normal. | Suppression ownership can oscillate and the UI may show intermittent corruption where none exists. Evaluate rolling freshness across a time window with expected update period, allowed duplicates, timeout, and recovery hysteresis. |
...
| SYS-002 | High | `g_brake_fault_active` is set in several paths but never cleared. The architecture does not define it as latched or give a reset rule. | UI recovery can remain red forever after the physical condition clears, with no way to distinguish intentional latch from bug. Define latch/reset semantics and expose cause plus first/last timestamps. |
...
| SYS-004 | High | Current SYS bench build defines `TESTING`; physical ESTOP, brake, mode, and light inputs are replaced by fixed values. The root build table also says bench uses `SYS_OWNS_MOTOR`, while [`sys-esp32/src/config.h`](sys-esp32/src/config.h) deliberately rejects that flag. | The bench profile cannot prove physical body-input wiring, and the documented build flags cannot compile as described. Define separate `simulation`, `CAN bench`, and `hardware bench` profiles with a capability manifest. Remove the retired flag from architecture. |
...
| MTR-004 | High | MTR does not run the YAML generator and consumes the hand-written protocol. | It can drift independently from RT/SYS and the UI. Include it in the generated-contract build and golden-vector tests. |
...
- RT high- and low-bus `0x7FD` heartbeats use independent counters in implementation and should remain independent in the UI.
