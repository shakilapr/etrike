...
`python protocol/tools/protocol.py validate` passes. That proves the committed generated files match the YAML input; it does **not** prove the firmware, host bridge, simulators, or documentation implement that contract.
...
|---|---|---|---|---|
| FRM-001 | High | [`can_high.yaml`](protocol/contracts/can_high.yaml) says RT state `0x210` is also sent on low, but [`can_low.yaml`](protocol/contracts/can_low.yaml) has no definition for it. | [`rt-esp32/src/main.cpp`](rt-esp32/src/main.cpp) transmits `0x210` on both high and low; SYS consumes the low copy. | A low-bus monitor sees an unknown/wrong-bus frame. Define one canonical `0x210` message and both transmit routes. |
