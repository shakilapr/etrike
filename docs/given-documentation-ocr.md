...
- **Protocol:** CAN_SES (SES/SEB proprietary steer-by-wire)
...
#### VCU → SES Command (ID: 0x169, cycle: 20 ms, DLC: 8)
...
> **⚠️ The exact byte layout, bit positions, scaling factors, and checksum algorithm must be verified against the project-specific protocol document before transmitting on hardware.** The `kSES/SEBCanOutputEnabled` flag in `config.h` defaults to `false` for this reason.

#### SES → VCU Status (ID: 0x201, cycle: 10 ms, DLC: 8)
...
**Product:** SES/SEB wire-controlled braking system (SEB)
...
> **⚠️ Same warning as SES:** verify byte layout and checksum algorithm before transmitting.
...
1. **Both SES and SEB use proprietary CAN protocols with checksums and rolling counters.** Do not transmit actuator commands until the byte layout, scaling, and checksum algorithm are confirmed against the project-specific protocol document from SES/SEB.

2. **SES requires zero-calibration** when paired with a new assembly. This is a one-time procedure, likely triggered via the `Alignment_Enable` flag in the command frame.
...
5. **SES and SEB are on the private CAN bus** (TWAI1 on the ESP32-S3). They must never be exposed to the public Jetson CAN bus.

6. **VCU address 0x27** appears in the DCDC spec for J1939 addressing. The SES and SEB protocols may use different addressing — verify with SES/SEB.
...
*Source PDFs: `given-documentation.pdf` (58 pages, SES/SEB SES + DCDC CAN spec), `given-documentation-ocr.pdf` (58 pages, alternate scan with higher-quality text layer). Both preserved in the project root for byte-layout verification.*
