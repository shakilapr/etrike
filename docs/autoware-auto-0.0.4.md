...
└────────────┬───────────────────────────────────────────┘
    ...
    ┌────────┼──────────┬──────────────┐
    ▼        ▼          ▼              ▼
  SES    SEB      SYS ESP32-S3   MTR STM32
...
|:---|:---|:---|:---|:---|:---|
| `0x200` STEER_CMD | `δ_rad = raw×0.001−π`<br>`δdot_rad_s = raw×0.01−10.0` | Dynamic clamp (speed-dep.)<br>Hard-stop ±40°<br>Following error 5°/300ms→ESTOP<br>LBS state machine | `eps_raw = δ_deg×10 + 30000` (offset=-3000 per SES/SEB CSV)<br>`eps_rate = clamp(δdot_deg_s, 125, 525)`<br>`veh_spd_kmh = measured_speed_mmps × 0.0036` | `0x169` VCU_SES_REQ @50Hz | SES/SEB SES |
| `0x201` BRAKE_CMD | `ratio = raw/10000`<br>`jetson_kpa = ratio×5000` | Max-select: `max(rt_obstacle_kpa, jetson_kpa)` | `seb_raw = kPa × 0.02` (clamp 0–100)<br>Mode: Pressure if >0, else Stroke | `0x205` RT_BRAKE_CMD→SYS→`0x7B9` VCU_SEB_REQ @50Hz | SYS→SES/SEB SEB |
...
**Locked (SES/SEB factory — cannot change):**
`0x169`, `0x201`, `0x202`, `0x203`, `0x6FA` (SES), `0x7B9`, `0x721`, `0x731`, `0x741`, `0x6FB` (SEB)

**Constrained (ours, must not collide with SES/SEB):**
...
/**:
  ros__parameters:
    ...
    # Steering limits
    max_steering_angle: 0.698    # rad (40° — SES software hard-stop)
    max_steering_rate: 9.16      # rad/s (525°/s)
    min_steering_rate: 2.18      # rad/s (125°/s — SES may reject below)
    ...
    # SES/SEB CAN IDs (for reference — actual TX by RT/SYS)
    SESmd_id: 0x169            # LOCKED
...
AckermannControlCommand.lateral.steering_tire_angle (rad)
  ...
  → SES: validate counter+cksum, PID to target
  → SES → 0x201 → RT: extract SES_StrAngle, following error check
...
- SES CSV says `offset=-3000` (raw=30000 at 0°). Architecture.md says `raw = mdeg/100` (raw=0 at 0°).
- **Fix:** Bench test with CAN analyzer. Turn SES to known angles, capture `0x201 SES_STATUS`. Trust CSV (manufacturer DBC) until verified.
...
| L4 | SES rate <125°/s: RT clamps floor at 125. Verify rejection behavior in Phase 4 |
...
- [ ] **B1:** Verify steering angle offset on live SES CAN bus
- [ ] **B2:** Verify SEB comm-fault behavior (hold vs release)
- [ ] Verify SES minimum slew rate (125°/s rejection)
...
| `0x169` | VCU_SES_REQ | RT | SES/SEB |
| `0x201` | SES_STATUS | SES | SES/SEB |
| `0x202` | SES_ErrInfo | SES | SES/SEB |
| `0x203` | SES_Version | SES | SES/SEB |
| `0x204` | RT_DRIVE_CMD | RT | — |
| `0x205` | RT_BRAKE_CMD | RT | — |
| `0x206` | MTR_MOTOR_FBK | MTR | — |
| `0x302` | HOST_LIGHT_CMD | RT (fwd) | — |
| `0x600` | SYS_DIAG_RPT | SYS | — |
| `0x6FA` | SES_Test | SES | SES/SEB |
| `0x6FB` | SEB_Test | SEB | SES/SEB |
| `0x721` | SEB_STATUS | SEB | SES/SEB |
| `0x731` | SEB_ErrInfo | SEB | SES/SEB |
| `0x741` | SEB_Version | SEB | SES/SEB |
| `0x7B9` | VCU_SEB_REQ | RT/SYS (mode-gated) | SES/SEB |
...
| **`0x201`/`0x202`/`0x203` collide with SES/SEB** | High bus (Jetson↔RT) vs low bus (actuators). Separate physical CAN networks. SES/SEB IDs are low-bus only and never forwarded (Category 3). |
