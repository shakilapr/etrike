...
| R1.5 | `kIdSES/SEBEpsStatus`, `kIdSES/SEBSebStatus` — code uses `kIdSES/SEBEpsStatus`, `kIdSES/SEBSebStatus` | Add aliases |
...
**Goal:** Brake actuation is exclusively through SES/SEB SEB via CAN `0x7B9`. The legacy GPIO-based `BrakeActuator` class is removed.
...
**Test:** Jetson publishes `/cmd_vel {linear.x=1.5, angular.z=0.2}` → `0x300` on high CAN → RT receives, resolves kinematics → `0x204` on low CAN → SYS MCP4725 outputs correct voltage → SES receives `0x169` angle command.
