...
| `MTR_MOTOR_FBK` | `0x206` | 4 | MTR ? RT, SYS, Host | `fault_flags` byte3: bit0 `kMtrFaultEstopActive` (0x01), bit1 `kMtrFaultCmdTimeout` (0x02), bit4 ready | MTR ESTOP **ack** (redundant path if SYS missed the `0x001`), plus command-timeout flag (issues #2/#7). `applied_speed_command_mmps` is the *echoed* command (not a measurement). |
