...
| RT steering ESTOP logic | Host test covers state behavior | Does not prove SES follows commands physically |
...
|------|-----------------------------|----------------------------|
| SES/SES | RT detects missing `0x201`; steering request enters safe state | Boot sync/listen resumes before steering is active |
...
|------|---------------------|
| SES / SES | Align, zero angle, positive/negative angle, clamp, following-error fault |
...
| SES CAN disconnected | Steering output behavior measured and documented |
