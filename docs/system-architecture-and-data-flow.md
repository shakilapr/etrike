...
- **Low CAN Bus (500 kbit/s)**: Primary vehicle actuation and safety bus connecting **RT**, **SYS** (Safety & Mode Manager), **MTR** (STM32G431 Motor & Relay Controller), **RM** (ESP32 FlySky RC Gateway), **SES** (SES Steer-by-Wire), **SEB** (Smart Electronic Brake), and auxiliary nodes (**PWT**).
...
```
 ...
 v               v                       v                v               v
┌─────────┐    ┌─────────────┐     ┌───────────┐    ┌───────────┐   ┌───────────┐
│SYS ESP32│    │  RM ESP32   │     │ MTR STM32 │    │ SES SES │   │  SEB Brake│
...
| **SES** | Smart Steer Actuator (SES) | Internal ECU | Closed-loop steering column positioning based on `0x169 VCU_SES_REQ`, status feedback on `0x201` | Does not compute steering trajectory or dynamic rollover bounds. |
