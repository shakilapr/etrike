...
On the E-Trike, the **RT ESP32-S3** is the gateway between the High-Level CAN (Jetson + RT) and Low-Level CAN (RT + SYS + SES/SEB actuators).
...
| `0x169` | VCU_SES_REQ | Low | RT output — steering angle → SES |
...
| `0x201` | SES_STATUS | Low | SES feedback — consumed by RT on low |
