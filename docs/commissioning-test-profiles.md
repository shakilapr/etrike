...
| Component bench test | An SES, SEB, motor controller, or sensor is tested directly by a test station |
...
|---|---|---|
| `SESOMPONENT_BENCH` | SES, constrained steering rack, CAN test station | Boot/alignment, center, direction, angle scaling, slew, limits, status, faults, counters/checksum, command loss, power cycling |
...
|---|---|---|---|
| `RT_CONTROLLER_BENCH` | RT and CAN analyzer only | Bench CAN traffic; actuator commands observed but physically disconnected | Test the complete RT board with simulated Host, SYS, SES, SEB, and MTR |
...
| `RT_STEERING_COMMISSIONING` | RT and SES only | Steering request only | Validate RT-to-SES alignment, angle commands, limits, following error, and ESTOP response |
...
| Gate | Connected hardware | Authority under test |
|---|---|---|
| Steering component acceptance | SES and test station | SES internal behavior and wire contract |
| RT steering integration | RT and SES | Alignment, bounded command generation, checksum/counter, and feedback |
| Following-error test | RT, SES, constrained rack | Persistent command/actual error detection |
| Command-loss test | RT and SES | Ramp, hold, center, or silence behavior defined by the safety design |
| Dynamic-limit test | RT, SES or actuator simulator | Speed-dependent and absolute angle limits |
...
- Use behavioral unit models, not mocks that always return the expected value.
  SES, SEB, MTR, Host, RT, and SYS simulators must model boot, healthy
...
|---:|---|---|
| 1 | SES only | Test station using `SESOMPONENT_BENCH` |
...
| 7 | RT plus SES only | `RT_STEERING_COMMISSIONING` |
