...
if (time_since_last_drive_cmd > 500 ms) {
    ...
    stop 0x200 VCU_SES_REQ        →  SES timeout-faults
}
```
...
While in AUTO mode, RT compares the commanded steering angle (`0x200`) against the actual angle reported by SES (`0x201 SES_StrAngle`):
...
- **CAN faults:** corrupted `0x200` frames (caught because checksum failure → SES rejects → angle doesn't move).
...
4. Steering: `0x200` stops transmitting → SES timeout-faults.
