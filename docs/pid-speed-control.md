...
void control_task() {
    ...
    while (true) {
        ...
        // Push to downstream (0x204 for MTR + 0x169 for SES)
        ActuatorOutput out = {
...
- **Steering angle:** SES runs its own internal position loop. RT commands an angle target; SES controls the motor to reach it.
