...
| `0x204` | [`RT_DRIVE_CMD`](file:///e:/work/etrike/can-dictionary.md#L119) | RT $\rightarrow$ SYS | 50 Hz | Computed motor speed setpoint (mm/s) and target gear (`N/D/S/R`). SYS monitors this for command-path consistency checks (EGAS L2 role) against the applied speed command (`0x206`, a setpoint echo — not a measured speed). |
...
4. **Steering Following-Error Check**:
   RT continuously monitors actual SES steering feedback angle vs commanded target ([safety_monitor.h](file:///e:/work/etrike/rt-esp32/src/safety_monitor.h#L123-L151)). If the error exceeds the dynamic speed-dependent threshold for $>100\text{ ms}$, RT aborts autonomous mode and latches ESTOP.
