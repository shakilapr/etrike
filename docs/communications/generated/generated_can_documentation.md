...
2. **Low-Level CAN Bus:** Connects the RT, SYS (Safety/Body controller), MTR (Motor controller), and Actuators (Steer-by-Wire SES and Brake-by-Wire SEB).
...
- **SES & SEB:** Smart steer-by-wire and brake-by-wire actuator modules relying on Intel byte-order sub-protocols.
...
| **SES** | steer-by-wire unit, steer-by-wire module, preprogrammed CAN IDs |
...
| `SteerDiag_Fault` | 2 | 0 | 8 | unsigned | 1 | [0, 1] | - | 0=OK, 1=SES fault |
| `SteerDiag_MotorCurrent` | 3 | 0 | 16 | unsigned | x0.01 | [0, 65535] | A | SES motor current, 0.01A/bit |
| `SteerDiag_ECUTemp` | 5 | 0 | 16 | unsigned | x0.1 | [0, 65535] | degC | SES ECU temperature, 0.1degC/bit |
...
- **DLC:** 5 bytes
- **Cycle:** 10 ms (0 = event-based)
- **Description:** MTR receives for motor actuation. SYS receives for EGAS L2 monitoring. ID 0x204 avoids collision with SES 0x202.
