...
Three physical CAN buses are planned: two at 500 kbit/s (high-level and low-level) and one at 250 kbit/s (powertrain). RT bridges selected messages between high and low buses. PWT low-to-powertrain bridging is a target design requiring additional CAN hardware. Actuators are CAN bus modules: SES (steer-by-wire) and SEB (electro-hydraulic brake). **Mode-gated dual control (Option D):** RT directly commands both SES and SEB in AUTO mode; SYS commands SEB in MANUAL mode (lever pass-through) and on ESTOP. Motor control remains blocked pending a complete MTR hardware implementation.
...
| SPI SCK | 15 | Out | MCP2515 (high CAN) |
...
| `0x201` SES_STATUS — DLC=8, `{u8 status, i16 angle, u8 torq, …}` | Steering SM: boot sync angle, alignment check, following-error monitor, steer-by-wire rolling counter + XOR checksum | `0x169` VCU_SES_REQ — DLC=8, `{u8 ctrl, i16 angle, u8 speed, u8 sec, u8 cnt+cksum, u8 cksum}` → SES (50 Hz, Angle Mode) | Steering angle |
...
**What RT controls:** SES steering angle via `0x169` (Angle Mode, 50 Hz, steer-by-wire rolling counter + checksum security), MTR motor speed + gear via `0x204` (100 Hz), SEB brake via `0x205`→SYS→`0x7B9` (50 Hz, Pressure/Stroke Mode). CAN gateway message forwarding between low and high buses.
...
| **12V accessory relay** | GPIO40. Cuts headlight, turn signals, mode bulbs during ESTOP. Brake light is on always-on DC-DC rail — not through this relay. | Prio 2 |
...
|------|-----------|--------|
| **Hardware** | Ignition GPIO (GPIO39) → main 12V relay | HIGH = 12V rail ON. LOW = all ECUs + actuators power down. Direct GPIO — no CAN, no software dependency. |
...
| `0x206` | MTR_MOTOR_FBK | `{i16 motor_command_speed_mmps, u8 gear_state, u8 fault_flags}` | MTR | Command-path consistency: compare speed setpoint vs applied command (echo); mismatch → ESTOP |
...
| Condition | CAN `0x012` | 12V accessory relay (GPIO40) |
|-----------|------------|------------------------------|
...
A momentary push button on GPIO11 (active-low, internal pull-up, debounced). Each press toggles the mode: MANUAL → AUTO → MANUAL. ESTOP cannot be exited via the MODE button — use the START button (GPIO41) or power-cycle.
...
mode_task @ 10 Hz:
  ...
  // Start button (GPIO41) — exit ESTOP
  read GPIO41
  ...
  prev_mode = GPIO11; prev_start = GPIO41
```
...
(GPIO41) or MODE long-press (3 s) transitions SYS to MANUAL mode, which
...
**Why only motor needs MTR:** Steering (SES) and brake (SEB) actuators are
...
DAC=0 V, relays OFF, SES centering, SEB idle.
...
behavior, and rolling counter freshness validation on the SES feedback path.
...
angle is written to the `0x169` command frame. The SES unit's internal
...
- **Obstacle ESTOP** (0x400 distance < threshold, Jetson ESTOP): hold the
  ...
  silent-stop `0x169` transmission. The SES enters comm-fault timeout and
...
**SES rolling counter freshness:**
The SES status frame (`0x201 SES_STATUS`) carries a 4-bit rolling counter
in byte 6 bits 4-7 (`roll_cnt_sts`). RT checks that this counter increments
on every frame. A frozen counter indicates a stuck SES CAN controller (the
...
This ordering is identical in both SES (0x201) and SEB (0x721) handling.
...
such as `CONFIG_BYPASS_SES_SYNC` (skip steering Listen-Before-Speaking for
bench testing without an SES unit). Every `platformio.ini` file that defines
