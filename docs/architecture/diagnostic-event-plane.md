...
| `0x0150` | `SYS_SES_STATUS_TIMEOUT` | SYS→SES | ESTOP | NOT_IMPLEMENTED | Low CAN monitor: complete absence of SES `0x201` during vehicle motion |
| `0x0151` | `SYS_SES_L3_FAULT` | SYS→SES | ESTOP | NOT_IMPLEMENTED | Low CAN monitor: decodes critical steer faults directly from SES `0x202` |
...
| `0x022D` | `RT_SUPPLY_VOLTAGE_SAG` | RT→SES | WARN | NOT_IMPLEMENTED | SES `0x6FA` supply voltage drops below $11.0$ V under actuator load |
...
| `0x0242` | `RT_SES_CONTROL_ENABLE_REJECTED`| RT→SES | WARN | NOT_IMPLEMENTED | SES fails to transition to active control mode feedback within 200 ms of command |
...
| **Steering / Brake Vendor Fault** (`*_SES_L3_FAULT`, `*_SEB_L3_FAULT`) | Vendor internal fault bitmask | Bitfield | Preserves proprietary SES / SEB error code for factory diagnostic tooling |
