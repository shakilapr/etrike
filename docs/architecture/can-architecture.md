...
+-----------------------+     | Real-Time Kinematics & Gateway|
         ...
         v                    v                               v                    v
 +---------------+    +---------------+               +---------------+    +---------------+
 | SYS ESP32-S3  |    | SES Steering|               |   SEB Brake   |    |  MTR (STM32)  |
...
- **Primary Function**: Shared vehicle control bus connecting **RT**, **SYS** (Safety & Mode Authority), **SES** (Steering), **SEB** (Braking), **MTR** (Motor Controller), and **HMI**.
...
- **Reason**: Legacy ESP-IDF driver automatically expanded zero-length frames (DLC 0) to 8 bytes. Modern handle-based API guarantees strict **0-byte wire contract** required by SEB and SES actuators.
...
- **`STEER_FAULT`**:
  ...
  - *Behavior*: Completely ceases transmitting `0x169` (`VCU_SES_REQ`), allowing SES to enter silent fallback.
...
- **Sneaky Trigger**: If Low CAN receives zero valid frames from any peer (e.g. SEB, SYS, SES are unpowered or disconnected during bench test).
