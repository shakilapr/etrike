...
| H5 | Steering/Front | JP1 → steering column | 1.0 m | SES power + CAN, front wheel encoder |
...
| **Deutsch DTP** | 2 | 25 A/contact | IP68 mated | 10–16 AWG | SES / SEB high-current 12 V |
...
| Fuse block (JP2) → SES | 12 | Red | 30 A peak | 1.0 m | 0.16 V |
...
| **Red** | 12 V switched | Accessory rail (via GPIO40 relay) |
...
Low-Level CAN Bus (500 kbit/s) — 6 nodes, linear multidrop:
  ...
  [120R]──RT────SYS────MTR────SES───SEB────DC-DC──[120R]
          |      |       |       |       |       |
...
| F_epsc | Forward fuse block (JP2), center | ATO | 30 A | 12 AWG to SES (1 m) | 12 V bus → SES → chassis at front |
...
| F_lights | Forward fuse block (JP2), center | ATO | 15 A | 14 AWG lighting bus (1.5 m) | 12 V bus → GPIO40 relay → lamps → chassis |
...
| DC-DC → SES | 12 V | 30 A | 12 AWG | 1.0 m | 0.16 V | 1.33% |
...
### 5.5 12 V Accessory Relay (GPIO40)

SYS GPIO40 controls a 40 A automotive relay:

```
GPIO40 ── 1 kΩ base R ── NPN (2N2222) ── relay coil ── 12 V
                                 │
...
7. **SES ECU housing** requires a chassis ground per the steer-by-wire manufacturer spec — this is for RF shielding, not power return. Bond the housing to chassis with a dedicated strap at its mounting point.
...
| SYS 12 V accessory | 1 | GPIO40 main accessory relay |
...
| 12 V high-current (SES, SEB) from signal cables | ≥50 mm |
...
Left Frame Rail (signal):           Right Frame Rail (power):
  ...
  Gear sense lines (20 AWG orange)     12 V SES/SEB (12 AWG red)
...
| Wire, 22 AWG yellow | 3 m | Relay coil drives (×6 + GPIO40) |
...
| Wire, 12 AWG red (GXL) | 1.5 m | JP2 fuse block → SES (1.0 m) + JP2 → SEB (1.5 m front→rear) |
| Wire, 12 AWG black (GXL) | 0.4 m | SES (−) → chassis strap; SEB (−) → chassis strap |
...
| ATO/ATC fuse block, 6-circuit | 2 | One at JP2 center (SES, Jetson, RT, SYS, lights, brake, CAN/MCU), one at rear (SEB, MTR) |
...
| DT04-4P + 0462-201-2031 | 6 | Deutsch | Low bus node drops (J10a–f): RT, SYS, MTR, SES, SEB, DC-DC |
...
| Item | Qty | Part Number | Notes |
|------|-----|-------------|-------|
| Wire, 12 AWG red | 1.5 m | SES 12 V power |
| Wire, 12 AWG black | 0.3 m | SES (−) → chassis strap (0.2 m) |
| Shielded pair, 22 AWG | 1.5 m | Belden 9841 | CAN drop |
| Shielded quad, 24 AWG | 1.5 m | Belden 8772 | Front wheel encoder (TBD sensor) |
| DTP04-2P + 0460-202-1631 | 1 | Deutsch DTP | SES power J21a |
| DT04-4P + 0462-201-2031 | 1 | Deutsch | SES CAN J21b |
...
| NUP2105L TVS | 1 | CAN protection | SES CAN node |
...
| Wire, 12 AWG black (GXL) | 0.5 m | SES (−) strap + SEB (−) strap (×2) |
...
| 0x169 | VCU_SES_REQ | RT | JP2 (center) | SES (H5, front steering) |
| 0x201 | SES_STATUS | SES | H5 (front steering) | RT (JP2) |
| 0x202 | SES_ErrInfo | SES | H5 (front steering) | RT (JP2) |
| 0x203 | SES_Version | SES | H5 (front steering) | RT (JP2) |
...
| 0x6FA | SES_Test | SES | H5 (front steering) | RT (JP2) |
