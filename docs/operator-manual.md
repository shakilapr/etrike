...
- **Steering:** Handlebar is mechanically linked to SES (standalone mode). RT does not transmit steering commands.
...
- **Steering:** RT receives 0x300 commands from Jetson, computes kinematics, transmits 0x169 to SES
...
4. **Listen Before Speaking (LBS):** Each actuator waits for the other side's status frame before transmitting commands:
   - Steering: RT waits for SES 0x201 status, reads current angle, syncs to it
