...
- SYS config: `sys-esp32/src/sys_config.h` (constants in `architecture.md` section 8.9)
...
- 72V bench power supply (SEB + SES power)
...
- SES steering unit with physical steering rack
...
Tests that verify fundamental hardware behavior of the SES/SEB actuators. These must pass before any powered integration test is conducted. They require only the actuator unit, a CAN analyzer, and a power supply -- no firmware needed.

---

### T1.1 — Steering Angle Offset Validation
- **Objective**: Verify SES responds correctly to commanded angles across full range with correct offset (raw 30000 = 0 deg).
- **Equipment**: SES unit, CAN analyzer (with 0x169/0x201 support), 12V power supply.
- **Procedure**:
  1. Power SES on bench CAN bus (12V, 500 kbit/s). Wait 2 s for boot.
...
- **Pass/Fail**:
  ...
  - +70 deg command: SES_StrAngle reads 37000 +/- 10 raw (unit capability). SES must NOT report L2/L3 error.
...
Tests that require all MCUs (RT, SYS, MTR) on a bench CAN bus with CAN analyzer. No physical actuators needed for the core timing measurements (though SEB/SES may be connected for end-to-end verification).
...
- **Procedure**:
  1. **Cold boot all simultaneously**: Power all nodes at once. Verify:
     ...
     e) Steering boot sequence: STEER_BOOT_WAIT (500 ms) -> STEER_LISTEN_SYNC -> STEER_ACTIVE (if SES present).
...
Tests that require all nodes and at least one physical actuator (SES or SEB) on the bench. These verify the safety mechanisms that depend on actuator feedback.
...
- **Equipment**: RT board, SES unit, CAN analyzer, variable resistor or mechanical stop to prevent SES from reaching commanded angle, 12V + 72V power.
- **Procedure**:
  ...
  2. **Low-speed test**: Command 30 deg at 2 km/h (555 mm/s). Dynamic limit at this speed = 40 deg, so 30 deg is valid. Following error threshold = max(2.0, 0.25 x 40) = 10.0 deg.
     a) Block SES from reaching beyond 5 deg (physical stop or by injecting fake 0x201 angle feedback).
  ...
  3. **High-speed test**: Command 10 deg at 25 km/h (6944 mm/s). Dynamic limit at this speed = 5 deg, so 10 deg would be clamped by RT to 5 deg. Following error threshold at 25 km/h = max(2.0, 0.25 x 5) = 2.0 deg.
     ...
     b) Block SES from reaching beyond 1 deg.
  ...
  4. **Mid-speed test**: Command 20 deg at 10 km/h (2778 mm/s). Dynamic limit at this speed = 40 - (10-2) x (35/23) = 40 - 8 x 1.52 = 27.8 deg. Following error threshold = max(2.0, 0.25 x 27.8) = 6.95 deg.
     a) Block SES at 2 deg.
     b) Measure t_follow_err.
  5. **Below-threshold test**: Same as low-speed test but allow SES to track within 8 deg error (below 10 deg threshold). Verify NO ESTOP triggers for 5 s.
- **Pass/Fail**:
...
- **Procedure**:
  1. Boot RT in AUTO mode with simulated Jetson 0x300 commands and SES on bench (or with simulated 0x201 feedback).
...
- **Equipment**: Full vehicle or full bench with SES, RT, SYS, CAN analyzer, obstacle simulator (0x400).
- **Procedure**:
  ...
  2. Command 30 deg steering angle (yaw rate producing > 40 deg bicycle model output, clamped by software hard-stop to 40 deg, but further clamped by dynamic clamp to ~5 deg at this speed).
     ...
     b) SES tracks to clamped angle; actual reads ~5 deg from 0x201.
...
- **Equipment**: RT board, SES unit, CAN analyzer, START button (SYS GPIO32), 12V power.
- **Procedure**:
...
- **Pass/Fail**:
  ...
  - After ramp completes (0 deg reached), steering may transition to ACTIVE (if SES aligned) or remain at 0 deg hold.
...
4. **Safety during test execution**: All bench tests involving 72V power (SEB, SES) must have a physical emergency disconnect within reach. For tests that inject CAN faults (T2.7), use a current-limited supply.
