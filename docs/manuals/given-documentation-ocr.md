# steer-by-wire SES & SEB — Technical Reference
...
1. **Both SES and SEB use proprietary CAN protocols with checksums and rolling counters.** Do not transmit actuator commands until the byte layout, scaling, and checksum algorithm are confirmed against the project-specific protocol document from steer-by-wire.
...
6. **VCU address 0x27** appears in the DCDC spec for J1939 addressing. The SES and SEB protocols may use different addressing — verify with steer-by-wire.
...
*Source PDFs: `given-documentation.pdf` (58 pages, steer-by-wire SES + DCDC CAN spec), `given-documentation-ocr.pdf` (58 pages, alternate scan with higher-quality text layer). Both preserved in the project root for byte-layout verification.*
