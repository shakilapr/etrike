...
| High | The SYS bench-only throttle input is impossible as currently mapped: firmware reads `ADC1_CHANNEL_5` (GPIO6 on ESP32-S3), GPIO6 is also the right-turn switch, while the documentation calls GPIO10 throttle and firmware drives GPIO10 as the headlight relay. | `sys-esp32/src/throttle_input.h`, `sys-esp32/src/config.h`. | Do not use `SYS_OWNS_MOTOR` with a real motor. Allocate a dedicated ADC-capable pin, update the ADC channel and wiring, and remove its conflicting function before bench actuation. |
