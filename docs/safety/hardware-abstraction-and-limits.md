...
This document details the architectural separation between the high-level autonomy stack (Jetson/Host) and the low-level physical actuators (SEB, SES, MTR). It explains the design philosophy behind the wide-range internal CAN messages versus the strictly clamped physical actuator limits, and how the intermediate microcontrollers (RT, SYS) enforce mechanical safety.
...
- **Physical Actuator Limits:** The SES actuator's CAN protocol can theoretically accept commands up to **±780°** (raw `±7800`). 
- **Mechanical Trike Limits:** The physical steering rack on the E-Trike bottoms out mechanically at approximately **±35° to ±45°**. 

### The Safety Clamp
Commanding the SES to turn to 780° when the rack stops at 45° would cause the steering motor to stall, eventually burning out the driver board or snapping the mechanical tie-rods. 
