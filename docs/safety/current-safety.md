...
*   **Steering Mechanical Limits (`rt-esp32/src/direct_resolver.cpp`):**
    Although the SES actuator allows commands up to ±780°, the E-trike's mechanical rack hits hard stops at ±45°. The kinematics resolver translates target yaw into an angle and strictly clamps it to `kSteerLimitMdeg` (±45000 mdeg) to prevent motor stall and tie-rod snapping.
...
In `MANUAL` mode, the RT node completely stops transmitting `0x169 VCU_SES_REQ` commands. This intentional silence forces the SES actuator to fall back into a "standalone" (free-spin) state, releasing motor resistance so the rider can physically turn the handlebars without the actuator fighting them.
```cpp
