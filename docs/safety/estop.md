...
| **Steering (SES)** | Obstacle: hold angle (dyn-clamped) ? silent-stop after 500ms. Non-obstacle: ramp to 0? at 20?/s via active 0x169. | Two-tier response. Fallback to silent-stop if following error persists >1s (mechanical jam). |
...
RT compares commanded steering angle (`0x169`) against actual angle reported by SES (`0x201 SES_StrAngle`) at 100 Hz.
...
Maximum allowable steering angle is inversely proportional to speed, enforced in RT's `physics_resolve()` before the command reaches SES:
...
### Additional Trigger: SES L3 Fault Escalation
...
SYS compares `0x204 RT_MotorSpeed` (commanded) vs `0x206 MTR_AppliedSpeedCmd` (feedback). Mismatch >500 mm/s persisting >500ms ? CAN `0x001` ESTOP.
...
5. Once centered, steering becomes fully manual (SES standalone).
...
Without the deferral, pressing START mid-ramp would stop `0x169` transmission immediately, leaving SES to comm-fault at whatever off-center angle the ramp had reached. A 30? steering lock in MANUAL mode (where SES is standalone) would make the vehicle unrideable.
...
3. SES enters internal comm-fault timeout.

**Why dynamic clamp applies:** During emergency braking while cornering, lateral load transfer reduces rear wheel grip. Large steering angle under hard braking approaches rollover threshold: `a_y = v?/L?tan(?) > g?w/(2h)`. The clamp prevents this.

**Straight-line case:** If ? ? 0? when obstacle detected, hold angle is 0? ? behavior identical regardless of clamp.
...
2. Continue transmitting during ramp so SES tracks centering.
...
4. SES timeout-faults internally.
...
- If neither works (SES remains fault-locked): **power-cycle** as final recovery.
...
|-----------|--------|------|-------|
| Steering | SES standalone (RT monitors 0x201, no TX) | RT active control via 0x169 | Obstacle: hold?silent. Non-obstacle: ramp?0??silent. |
...
### SES (Steering) LBS Parameters

| Parameter | Value |
|-----------|-------|
| Status ID | `0x201` SES_STATUS (100 Hz from SES) |
| Position field | `SES_StrAngle` (u16, 0.1?/bit, offset -3000) |
| Alignment field | `SES_INF_Angle_Status` (bit 0 of byte 0) |
| Command ID | `0x169` VCU_SES_REQ (50 Hz to SES) |
| Boot wait | 500ms |
...
- **SES:** No `0x169` for >20ms ? internal comm fault ? locks or goes limp.
...
| **SYS heartbeat lost** (RT detects) | 0V (MTR local) | N (MTR local) | Max (RT takeover 0x7B9) | SES timeout* (RT still alive) | ON (RT alive) | OFF | ON | SYS reboot?re-sync; MANUAL |
...
| **Watchdog reset** | 0V (MTR local) | N (MTR local) | SEB timeout* | SES timeout* | OFF?ON | OFF | OFF?ON | Reboot?MANUAL |
