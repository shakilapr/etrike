...
| Safety Goal | Functional Req | Technical Req | Implementation | Test | Verification Evidence | Status |
|-------------|---------------|---------------|---------------|------|---------------------|--------|
| SG-04: Actuator comms loss must be detected within 100ms | FR-14: RT must detect 0x201 SES status timeout | TR-14: SES timeout detection at 20ms threshold | `rt-esp32/src/steering_control.h` — SES status timeout | `test_steering_control.cpp` — timeout test | Unit test | Verified |
...
| SG-04 | FR-16: Rolling counter must detect frozen actuator | TR-16: SES and SEB rolling counter freshness monitored | `rt-esp32/src/steering_control.h` — frozen rolling counter detection | `test_components.cpp:139-142` — rolling counter test | Unit test | Verified |
...
| 7.5 | SES 0x201 rolling counter not checked for frozen counter | `rt-esp32/src/steering_control.h` | HAZ-04 | SG-04 | Verified |
...
| 7.5 | N/A — frozen rolling counter guard added per commit `cd9209b` | `Add frozen rolling-counter guard for SES 0x201 feedback` |
