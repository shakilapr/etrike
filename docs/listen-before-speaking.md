...
- **SES** (steering): runs an internal position loop. If you send `0x200` with angle=0° and the wheel is physically at 15°, it will slew to 0° at maximum motor speed. This can yank the handlebars out of the rider's hands.
...
| Command ID | `0x200` VCU_SES_REQ (50 Hz to SES) |
| Boot wait | 500 ms |
| Sync timeout | 2 seconds → STEER_FAULT, remain in MANUAL |
| Mode behavior | MANUAL: do NOT send `0x200`, SES standalone. AUTO: send at 50 Hz. ESTOP: stop sending. |
...
- SES: if `0x200` stops for >20 ms → internal comm fault → locks or goes limp (TBD by unit spec).
