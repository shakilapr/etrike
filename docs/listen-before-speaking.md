...
This is used for both SES/SEB actuators on our tricycle.
...
- **SES** (steering): runs an internal position loop. If you send `0x169` with angle=0° and the wheel is physically at 15°, it will slew to 0° at maximum motor speed. This can yank the handlebars out of the rider's hands.
...
| Mode behavior | MANUAL: do NOT send `0x169`, SES standalone. AUTO: send at 50 Hz. ESTOP: stop sending. |
...
void steer_state_machine() {
    ...
    case STEER_FAULT:
        // Silent — let SES timeout-fault internally
        break;
    }
}
```
...
SES/SEB actuators run an internal bootloader and self-test on power-up. During this window:
...
2. Controller boots faster than SES and immediately commands 0°.
3. SES slews 40° at max motor speed — handlebars whip to center.
...
- SES: if `0x169` stops for >20 ms → internal comm fault → locks or goes limp (TBD by unit spec).
...
*See also: [[defense-in-depth-safety]] for following error and dynamic angle clamp, [[SES/SEB-security-protocol]] for rolling counter + checksum, [[architecture]] §7.6 for the steering boot sequence, §8.6 for the brake boot sequence.*
