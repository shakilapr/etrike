...
| Steering decoding | ⚠️ Offset risk | `0x310` STEER_DIAG: code uses `offset=-3000` (30.0°). SES/SEB SES CSV spec may differ. Pre-existing issue B1 in v0.0.4 audit. |
...
| **Steering actuation** | CAN command → steering ECU | `0x169` VCU_SES_REQ → SES/SEB SES (Angle Mode, 50 Hz, rolling counter + checksum) | ✅ Proprietary but validated |
| **Brake actuation** | CAN command → brake ECU | `0x7B9` VCU_SEB_REQ → SES/SEB SEB (Pressure/Stroke Mode, 50 Hz, rolling counter + checksum). RT→`0x205`→SYS→`0x7B9` (AUTO) or SYS→`0x7B9` (MANUAL/ESTOP). | ✅ Mode-gated dual sender |
...
| AckermannControlCommand → steering angle | 2 CAN frames | ~20 ms | Jetson→`0x300`→RT→kinematics→`0x169`→SES (50 Hz steering loop) |
...
| Steering feedback → SteeringReport | 2 CAN frames | ~20 ms | SES→`0x201`→RT→`0x310`→Jetson (10 Hz steering diag) |
...
`0x310` STEER_DIAG decoding uses `offset=-3000` (30.0°). The SES/SEB SES CSV spec must be verified. A wrong offset produces a constant steering angle bias in `SteeringReport` and dead-reckoning odometry. **Pre-existing issue B1** from v0.0.4 audit.
