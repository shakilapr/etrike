...
**Why this matters:** Without the deferral, pressing START mid-ramp would stop `0x169` transmission immediately, leaving SES to comm-fault at whatever off-center angle the ramp had reached. A 30° steering lock in MANUAL mode (where SES is standalone and provides no active centering) would make the vehicle unrideable. See [[emergency-safety-analysis]] §1 for the full causal trace.
...
This catches stuck linkage, rock jams, bent tie rods, SES motor failure, encoder failure, and corrupted CAN frames.
...
2. Continue transmitting during the ramp so SES tracks the centering.
...
**Recovery:** After exiting ESTOP to MANUAL, the steer SM is in `STEER_FAULT`. The rider has two recovery options (same as SES boot sync failure):
...
| RT CAN TX fails | TWAI TX errors | Log, auto-recover; persistent → SES timeout-fault |
...
- Steering still responds to handlebars (SES standalone in MANUAL).
...
| CAN frame corruption | SES/SEB checksum fail | Frame silently discarded. Rolling counter NOT incremented. |
...
| Comm timeout | No `0x169` for >20ms | SES internal fault lock |
...
| **Watchdog reset** | 0V (MTR kills locally) | N (MTR cuts locally) | SEB timeout* | SES timeout* | OFF | OFF | All OFF | Reboot → MANUAL |
...
| [[SES/SEB-security-protocol]] | Rolling counter + XOR checksum for SES/SEB actuators |
