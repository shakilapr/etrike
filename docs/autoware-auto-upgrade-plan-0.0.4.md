...
**Previous version (0.0.3-alpha):** Fixed drive-by-wire codes (CSV ↔ architecture alignment, CAN ID corrections, SES/SEB protocol signals)
...
| Tier 3 Actuation ECUs | RT ESP32-S3 (CAN gateway) | RT receives high-bus commands, translates to low-bus SES/SEB + custom CAN IDs |
...
| Document CAN ID | Document Purpose | Our Current ID | Our Current Purpose | Action |
|:---|:---|:---|:---|:---|
| `0x300` | Actual steer angle + rate + fault | (none — RT monitors `0x201` SES internally) | — | **Adopt.** RT translates SES `0x201` → high-bus `0x300`. |
...
|:---|:---|:---|:---|:---|
| `0x200` Steering angle | `raw = (δ_rad + π) / 0.001` → int16 | Tire angle in radians, offset by +π to keep raw positive. δ=0 (straight) → raw≈3142. δ=+0.698rad (40° right) → raw≈3840. δ=-0.698rad (40° left) → raw≈2444. | `raw = (control.lateral.steering_tire_angle + M_PI) / 0.001` | Extract `δ_rad = raw × 0.001 - π` → convert to SES decideg: `eps_raw = δ_deg × 10` (plus verify offset=-3000 per CSV). Apply dynamic clamp + LBS SM. |
| `0x200` Steering rate | `raw = (δdot_rad_s + 10.0) / 0.01` → int16 | Slew rate in rad/s, +10 offset. SES range 2.18–9.16 rad/s → raw 1218–1916. | `raw = (control.lateral.steering_tire_rotation_rate + 10.0) / 0.01` | Extract `δdot_rad_s = raw × 0.01 - 10.0` → convert to °/s for SES `VCU_SES_Tgt_StrAngleSpd`. Clamp to 125–525 °/s. |
...
|:---|:---|:---|:---|:---|
| `0x300` Steer angle | `θ_rad = raw × 0.001 - π` → int16 | Inverse of command encoding. δ=0 → raw≈3142. | Extract SES angle (decideg) → `δ_rad = angle_deg × π/180` → `raw = (δ_rad + π) / 0.001` | `steering_tire_angle = raw × 0.001 - π` → publish SteeringReport |
| `0x300` Steer rate | `θdot_rad_s = raw × 0.01 - 10.0` → int16 | Inverse of command encoding. | Extract SES rate (°/s) → `δdot_rad_s = rate_deg_s × π/180` → `raw = (δdot_rad_s + 10.0) / 0.01` | Published in SteeringReport |
...
| SES/SEB protocol reference | §2 (new subsection) | Cross-reference to `can-dictionary.md` for low-bus details |
...
| **SES/SEB IDs** | `0x169`, `0x201`, `0x202`, `0x203`, `0x6FA`, `0x7B9`, `0x721`, `0x731`, `0x741`, `0x6FB` | Low | ❌ **No** — factory preprogrammed. |
| **Our low-bus IDs** | `0x011`, `0x012`, `0x110`, `0x120`, `0x204`, `0x205`, `0x206`, `0x302`, `0x600`, `0x7FD`, `0x7FE`, `0x001` | Low | ⚠️ **Constrained** — ours but must not collide with SES/SEB. |
...
- Document's `0x201` (brake cmd, high bus) and SES `0x201` (status, low bus) coexist safely — RT receives the command on MCP2515/SPI, and separately receives SES status on TWAI.
- Document's `0x202` (throttle cmd, high bus) and SES `0x202` (error info, low bus) — same separation.
- Document's `0x203` (gear cmd, high bus) and SES `0x203` (version, low bus) — same.
...
2. RT translates between document's high-bus IDs and the low-bus SES/SEB + custom IDs
...
|:---|:---|:---|:---|
| `0x200` Steering cmd `{i16 angle_deg, i16 rate_deg_s, u8 enable}` | Convert angle to SES raw format. Apply dynamic clamp. Run LBS state machine. Build SES/SEB frame with rolling counter + checksum. **⚠️ Steering offset TBD:** SES CSV uses offset=-3000 encoding. Must verify against live CAN bus before Phase 5. Also populate `VCU_Veh_Spd_Value` (byte 6) — convert mm/s → km/h via `kmh = mmps × 0.0036`. | `0x169` VCU_SES_REQ @50Hz | SES/SEB SES |
| `0x201` Brake cmd `{u16 pressure_kpa, u8 enable}` | Max-select with obstacle emergency. Convert kPa→SEB raw via `raw=kpa×0.02`. Mode=Pressure if >0, else Stroke. | `0x205` RT_BRAKE_CMD → SYS → `0x7B9` VCU_SEB_REQ @50Hz | SYS ESP32 → SES/SEB SEB |
...
|:---|:---|:---|
| `0x201` SES_STATUS (SES) | Extract angle, rate, fault. Convert decideg→deg. | `0x300` Steer feedback `{i16 angle_deg, i16 rate_deg_s, u8 fault}` @100Hz |
...
| `0x320` | SPEED_YAW_FEEDBACK | 4 | `{i16 speed_mmps, i16 yaw_rate_mrad_s}` — **NEW for Autoware.Auto VelocityReport.** Yaw: TBD (derived from SES angle+speed via inverse kinematics, or IMU when fitted, or 0). | 100 Hz |
...
| `max_steering_angle` | 1.0 rad | **0.698 rad (40°)** | SES software hard-stop |
| `max_steering_rate` | 5.0 rad/s | **9.16 rad/s (525°/s)** | SES max slew rate |
| `min_steering_rate` | *(not in original)* | **2.18 rad/s (125°/s)** | SES minimum — commands below this may be rejected |
...
> 
> The RT ESP32-S3 translates between this document's high-level CAN protocol (§2) and the vehicle-specific low-level CAN bus (SES/SEB SES, SEB, MTR STM32, SYS ESP32-S3). It runs 8 FreeRTOS tasks with NO ROS 2 middleware. For the complete RTOS task layout, steering Listen-Before-Speaking state machine, tricycle kinematics algorithm, and CAN gateway forwarding rules, see [`architecture.md`](../architecture.md) §7.
...
| *(missing)* → SES/SEB protocol safety | Rolling counter + checksum on `0x169` and `0x7B9`. Both enable bits must be 1. |
...
| H5 | **`loop_rate: 30 Hz` too slow** — SES/SEB requires 50 Hz continuous; RT runs 100 Hz | §3 line 97 | Change to 100 Hz. |
...
- [ ] Add missing parameters (tricycle wheelbase, SES/SEB IDs, `is_defined_*` flag checks)
...
- [ ] **BLOCKER: Verify steering angle encoding** — capture live CAN bus traffic from SES to confirm offset encoding (CSV offset=-3000 vs architecture offset=0). Phase 5 cannot proceed until resolved.
...
|:---|:---|:---|
| **Steering angle offset ambiguity** — SES CSV uses offset=-3000 encoding; architecture.md uses offset=0. If wrong encoding is used, steering commands are catastrophically wrong. | **BLOCKER** | Must capture live CAN bus traffic from physical SES unit and verify encoding before Phase 5. CSV (manufacturer DBC export) is the authoritative source — likely offset=-3000 is correct. |
...
| **`0x320` yaw rate** — no sensor currently provides yaw. | **MEDIUM** | Derive from SES angle + speed (inverse kinematics), IMU (future, sensor TBD), or set to 0. Mark as TBD. |
| **`min_steering_rate` 125°/s** — SES may reject commands below this. | **MEDIUM** | Added as parameter. RT must ensure commanded rate ≥ 125°/s. |
...
| **Checksum algorithm** — XOR vs SUM ambiguous across SES/SEB docs. | **LOW** | Use XOR(bytes 0–6) ^ 0xFF per can-dictionary.md. Verify against physical unit. |
...
| D4 | Steering rate type: plan uses i16, SES/SEB expects u16. Functionally identical for 125–525 range. Noted. |
