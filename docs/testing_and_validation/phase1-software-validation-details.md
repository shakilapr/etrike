...
| Hardware forbidden | RT board, SYS board, CAN analyzer, CAN transceiver, SES/SES, SEB, MTR, PWT, vehicle wiring, vehicle power |
...
| `simulation/tests/unit/epsc.test.ts` | SES behavior |
...
|------|---------------------|--------------------|
| SES/SES | RT detects missing `0x201`; steering safe state | Boot sync/listen resumes before steering active |
...
| SES/SES | Sends steering feedback/status and follows steering commands in virtual plant | Missing feedback, frozen angle, following error, reconnect with old counter |
...
| `g_bypass_eps_sync` | RT/SYS globals | Skip SES listen/sync path | Must only work with bench/software mode, never production |
...
| `simulation/src/phase1/mimics/ses.ts` | SES/SES | Emits steering feedback/status; follows or fails to follow commands |
