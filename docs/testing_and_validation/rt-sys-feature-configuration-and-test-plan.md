...
- presence and test policy for Host, HMI, MTR, SES, SEB, PWT, DC-DC, and the
...
| SES / SES | Low CAN | RT | Physical steering actuator and feedback unit |
...
These effects must remain separate. Marking SES physical does not by itself
authorize steering output. A controller-only build may simulate SES while all
actuator output remains inhibited. A steering bench may require physical SES
...
- sends steering requests directly to SES;
...
`Simulated` does not mean that an SES, SEB, MTR, Host, HMI, PWT, or DC-DC model
...
| `ETRIKE_UNIT_SES` | RT | SES / SES steering unit |
...
physical Host, SYS, MTR, SES, SEB, PWT, DC-DC, and motor-controller policies
...
Example isolated RT plus SES settings allow only
...
|---|---|---|---|
| RT controller only | Host/SYS/MTR/SES/SEB simulated as needed | HMI/PWT/DC-DC/motor controller unless under test | None |
| SYS controller only | RT/MTR/SEB simulated as needed | Host/SES/PWT/DC-DC/motor controller | None or protected SYS body dummy loads |
| RT plus SES | SES physical; Host/SYS simulated as needed | MTR/SEB/PWT/DC-DC/motor controller | RT steering only |
| SYS plus SEB | SEB physical; RT simulated as needed | MTR/SES/PWT/DC-DC/motor controller | SYS brake only |
| RT plus SEB | SEB physical; SYS simulated as needed | MTR/SES/PWT/DC-DC/motor controller | RT direct brake only |
| RT/SYS plus MTR | MTR physical; RT/SYS physical | SES/SEB/PWT/DC-DC unless separately accepted | RT drive to MTR; MTR motor output initially inhibited |
| MTR plus motor controller | MTR and motor controller physical | SES/SEB/PWT/DC-DC | MTR motor only |
| PWT plus DC-DC | PWT and DC-DC physical on powertrain bus | RT/SYS/MTR/SES/SEB not connected to that bus | PWT DC-DC only |
...
| Policy | Required behavior |
|---|---|
| Disabled | RT suppresses steering requests and reports autonomous steering unavailable; disabling SES must not implicitly unlock drive |
...
| Simulated | SES model implements status, checksum/counter, alignment, command following, faults, and command loss |
...
steering output with SES disabled, and drive output with MTR disabled.
...
Run the following contract for Host, HMI, MTR, SES, SEB, PWT, DC-DC, and the
...
- all unit policies: Host, SYS, HMI, MTR, SES, SEB, PWT, DC-DC, and motor
  controller;
...
- SES-only, SEB-only, MTR-only, HMI-only, and PWT/DC-DC isolated setups;
...
- no SES, SEB, MTR, PWT, DC-DC, motor controller, or powered encoder.
...
### Gate 3: SES steering unit

Connected:

- RT;
- SES and mechanically constrained steering rack;
- CAN analyzer/test station;
...
- Execute SES-only steering tests.
...
- Host, HMI, MTR, SES, SEB, PWT, DC-DC, and motor-controller policies are
