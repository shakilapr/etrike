...
| High | The controllers are ESP32-S3 N16R8 modules: 16 MB flash and 8 MB octal PSRAM. The project now uses a local `esp32-s3-devkitc-1-n16r8` board definition; verify the module marking and flash ID before programming. | RT/SYS `platformio.ini` and local `boards/esp32-s3-devkitc-1-n16r8.json`. | Keep the N16R8 board and ESP-IDF memory configuration aligned. Do not flash an unverified module or assume PSRAM wiring from a different board variant. |
