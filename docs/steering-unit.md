# SES/SEB SES — Steer-by-Wire Unit
...
### Unit conversion (internal mdeg ↔ SES/SEB raw, offset -3000)

The CSV defines an offset of -3000 for `VCU_SES_Tgt_StrAngle`, meaning 0° corresponds to a raw value of 30000.

```
SES/SEB raw = (internal_angle_mdeg / 100) + 30000    (45500 mdeg → 455 + 30000 = 30455 raw → 45.5°)
internal_mdeg = (SES/SEB raw - 30000) × 100          (30455 raw → 455 × 100 = 45500 mdeg → 45.5°)
```
...
Command the steering rack to a specific angle. The SES's internal PID drives the motor until the built-in angle sensor matches the target.
...
These are implemented in the RT firmware, not in the SES unit itself.

### 5.1 Software Hard-Stops

The SES accepts commands up to ±780° (raw ±7800). The physical trike steering rack maxes out around ±35–45°. Commanding beyond the mechanical limit will stall the motor, potentially burning out the driver or snapping a tie-rod.
...
If the physical wheel gets stuck (rock jam, linkage failure) while the SES tries to turn:
...
Same as SEB: 4-bit value (0–15), increment every frame. Two consecutive frames with same counter → SES rejects, triggers fault.
...
Unlike the SEB, the SES requires explicit enable bits to activate its security checks:
...
The SES has an absolute encoder that retains calibration across power cycles. No physical homing sweep is required on every boot. However, a strict software handshake is mandatory.
...
STEER_FAULT:
  ...
  - SES will timeout-fault (lock or limp — verify behavior with SES/SEB spec)
```
...
- **Required:** Only when the SES is first mated to a new mechanical assembly.
...
void ses_send_command(float target_angle_deg, uint16_t speed_limit_deg_s) {
    ...
    // 3. Angle target (internal mdeg → SES/SEB raw with offset -3000)
...
| Condition | SES Response | Controller Action |
|-----------|---------------|-------------------|
...
| SES timeout-fault behavior | TBD — lock, center, or freewheel | Verify against SES/SEB spec |
...
- **Not reconfigurable.** Factory CAN IDs are fixed. SES command `0x169` (customized from factory default `0x200`). `RT_DRIVE_CMD` is at `0x204` to avoid collision.
- **MANUAL mode:** RT does not send `0x169`. SES operates standalone from rider steering wheel input.
- **AUTO mode:** RT sends `0x169` at 50 Hz with resolved angle + dynamic clamp + slew rate.
- **ESTOP:** RT stops sending `0x169`. SES timeout-faults (behavior TBD).
