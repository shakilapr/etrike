...
internal_mdeg = SES/SEB raw × 100          (455 raw → 45500 mdeg → 45.5°)
```
...
void ses_send_command(float target_angle_deg, uint8_t speed_limit_deg_s) {
    ...
    // 3. Angle target (internal mdeg → SES/SEB decideg)
...
- **Not reconfigurable.** Factory CAN IDs are fixed. SES command `0x200` is the factory default — `RT_DRIVE_SETPOINT` was moved to `0x202` to resolve the collision.
- **MANUAL mode:** RT does not send `0x200`. SES operates standalone from rider steering wheel input.
- **AUTO mode:** RT sends `0x200` at 50 Hz with resolved angle + dynamic clamp + slew rate.
- **ESTOP:** RT stops sending `0x200`. SES timeout-faults (behavior TBD).
