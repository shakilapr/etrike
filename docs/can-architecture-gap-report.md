...
| `kEstopRateLimitWindowMs` (TX vs RX conflated) | 500 ms / 2 frames | TX: `kEstopBroadcastMinIntervalUs=250'000 ?s` per ECU; RX: 500 ms/2 in SYS dispatch ? **two different mechanisms** | `shared_config.h:26`; `sys-esp32/src/main.cpp:287-298` |
...
- Angle plausibility `|?_boot| ? 30?` (300 raw / 0.1? units) ?
