...
- **Low bus:** RT TWAI ↔ SYS TWAI ↔ MTR ↔ PWT TWAI0 ↔ SES ↔ SEB ↔ DC-DC. 120 Ω at both physical ends.
...
|---|--------|------|------|-------------|--------|-------|
| 1 | CAN RX (low bus) | 4 | TWAI RX | SN65HVD230 CRX | **Active** | Low bus frames from SYS, MTR, SES, SEB |
...
- **Termination:** If SES is at a physical end of the low bus, enable 120 Ω. Otherwise leave off.
...
- **12V Fuse Block** → ESP32-S3 Dev Boards (×4), STM32 Board, SES, SEB, Jetson Orin NX (via separate 19V DC-DC)
...
| 12 V DC | DC-DC converter | Lamps, bulbs, relay coils, SES, SEB, ECU boards (via onboard regulators) | 12V relay (SYS GPIO40) |
...
| **Low** | SES | ❌ OFF | — | Middle tap (unless at physical end) |
...
| RT steering FAULT | No 0x201 (SES status) on low bus | Inject 0x201 or connect SES |
