...
```
  RT-ESP32 (TWAI)          SYS-ESP32 (TWAI)       SES          SEB         DC-DC
...
|---|--------|------|------|-------------|-------|
| 1 | CAN RX (low bus) | 4 | TWAI RX | SN65HVD230 RXD pin | Low-level CAN: SYS, SES, SEB, DC-DC |
...
| # | Signal | GPIO | Type | Connected To | Notes |
|---|--------|------|------|-------------|-------|
| 1 | CAN TX (low bus) | 5 | TWAI TX | SN65HVD230 TXD pin | Commands to SYS + SES |
...
72V Traction Battery
  ...
  │
  ├── SES/SEB SES (steering) — internal power
  ├── SES/SEB SEB (brake) — internal power
  │
