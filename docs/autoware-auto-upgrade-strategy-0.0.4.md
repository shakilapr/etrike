...
|:---|:---|:---|:---|:---|:---|
| `0x200` STEER_CMD | `δ_rad = raw × 0.001 − π`<br>`δdot_rad_s = raw × 0.01 − 10.0` | Dynamic angle clamp (speed-dependent)<br>Software hard-stops (±40°)<br>Following error check | `eps_angle_raw = δ_deg × 10` (plus verify offset=-3000 per SES/SEB CSV)<br>`eps_rate = δdot_deg_s` (clamp 125–525 °/s) | `0x169` VCU_SES_REQ @50Hz | SES/SEB SES |
| `0x201` BRAKE_CMD | `ratio = raw / 10000.0f` | Max-select: `max(rt_obstacle_pressure, ratio × 5000 kPa)` | `kPa = ratio × 5000` → `seb_raw = kPa × 0.02` | `0x205` RT_BRAKE_CMD → SYS → `0x7B9` VCU_SEB_REQ @50Hz | SYS → SES/SEB SEB |
...
**Steering conversion detail (BLOCKER):** The SES/SEB SES CSV declares `VCU_SES_Tgt_StrAngle` with `scale=0.1, offset=-3000` (unsigned 16-bit). This means: `raw = δ_deg × 10 + 30000`. At 0° → raw=30000, at +40° → raw=30400, at -40° → raw=29600. This encoding must be verified against live CAN bus traffic from the physical SES unit before Phase 5 implementation.

### L2.3 RT Feedback Translation (Trike Physical → Ratio)

| Low CAN Source | RT Extracts | Converts to Ratio | High CAN Output |
|:---|:---|:---|:---|
| `0x201` SES_STATUS (SES) | `SES_StrAngle` (u16, offset=-3000 per CSV) → `δ_deg` | `δ_rad = δ_deg × π/180` → encode: `raw = (δ_rad + π) × 1000` | `0x300` STEER_FBK @100Hz |
...
/**:
  ros__parameters:
    ...
    max_steering_rate: 9.16      # rad/s (525°/s — SES max)
    min_steering_rate: 2.18      # rad/s (125°/s — SES min, may reject below)
    ...
    SESmd_id: 0x169            # VCU_SES_REQ (SES/SEB factory — LOCKED)
    eps_status_id: 0x201         # SES_STATUS
    seb_cmd_id: 0x7B9            # VCU_SEB_REQ (SES/SEB factory — LOCKED)
...
**Constrained (ours, on low bus, must not collide with SES/SEB):**
...
**High bus and low bus are physically separate.** RT has two different CAN controllers (TWAI for low, MCP2515/SPI for high). Same CAN ID on both buses is safe — RT distinguishes by controller. But for debugging clarity, high-bus IDs use `0x2xx`/`0x3xx` namespace, low-bus IDs are scattered per SES/SEB assignment.
...
### Phase 4: SES/SEB Verification (BLOCKER)
- [ ] **BLOCKER:** Capture live CAN bus traffic from physical SES
- [ ] Verify steering angle encoding (offset=0 vs offset=-3000)
- [ ] Verify SES minimum slew rate (125°/s rejection threshold)
