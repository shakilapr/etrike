...
- **MTR, SES, SEB are absent** — we inject their frames on the low bus.
...
| **SES (steering)** | Low | `0x201` | RT steering state machine — prevents FAULT after 5s |
...
# Mimics Host (high bus) + MTR + SES + SEB (low bus) for RT↔SYS bench test.
...
$lowBus = @(
  ...
  @{ bus="low";  id="0x201"; dlc=8; data=@(1,0,0,0,0,0,0,0); ms=10; desc="SES status (angle=0, OK)" },
...
| **SES (steering)** | Low | `0x201` (100 Hz), `0x202` (10 Hz) | `0x201` min | RT steering needs `0x201` angle feedback. Without it, LISTEN_SYNC times out after 5s → FAULT. |
...
| RT steering FAULT | No `0x201` (SES status) injected | Start `0x201` periodic injection (section 7) |
...
No Host, MTR, SES, or SEB connected.
