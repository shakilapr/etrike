...
- **MTR, SES, SEB are absent** — we inject their frames via CANalyst-II.
...
# Mimics MTR + SES + SEB on the low CAN bus for RT↔SYS bench testing.
...
$periodics = @(
  ...
  # ── SES (steering actuator) ────────────────────────────────────
  @{ id="0x201"; dlc=8; data=@(1,0,0,0,0,0,0,0); ms=10; desc="SES status (angle=0, OK)" },
...
| **SES (steering)** | `0x201` (100 Hz), `0x202` (10 Hz), `0x203` (1 Hz), `0x6FA` (100 Hz) | `0x201` minimum | RT steering state machine needs `0x201` to track angle. Without it, steering stays in LISTEN_SYNC, times out after 5s → FAULT. |
...
| RT steering FAULT | No `0x201` (SES status) injected | Start `0x201` periodic injection (section 5) |
