# SES/SEB CAN Security Protocol — Rolling Counter + Checksum

SES/SEB SES and SEB actuators require two security bytes in every command frame: a **rolling counter** and an **XOR checksum**. If either is wrong, the actuator silently discards the frame — no error response, no fault flag, just ignored.
...
SES/SEBSecurityFrame cmd;

// SES steering command
...
When SES or SEB receives a frame:
...
4. Check that byte ordering matches SES/SEB's little-endian expectation.
...
SES/SEB chose XOR + inversion over a proper CRC:
...
The downside: XOR catches single-bit errors but is weaker against multi-bit errors than CRC-8 or CRC-16. SES/SEB's design trades stronger error detection for simplicity and the addition of liveness checking via the counter.
...
*See also: [[listen-before-speaking]] for the boot sequence that must complete before security bytes matter, [[architecture]] §7.6 for SES protocol, §8.6 for SEB protocol, [[can-dictionary]] for bit-level frame layouts.*
