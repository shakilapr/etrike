...
- Host, HMI, SYS, MTR, SES, SEB, PWT, DC-DC, and motor-controller policies are
...
| SES / SES | Steering actuator and feedback | Physical acceptance incomplete | 1, 5, 13, 15-18, 20, 24, 27 |
...
| 20 | SES physical integration | SES only |
...
- Implement SES disabled/physical/simulated policy.
...
- Disabled SES does not unlock drive.
...
- Add Host, HMI, RT, SYS, MTR, SES, SEB, PWT/DC-DC models as required.
...
- Keep SES, SEB, MTR outputs, motor controller, PWT, and DC-DC disconnected.
...
- Simulate Host/HMI/MTR/SES/SEB only when those units are absent.
...
## Phase 20 - SES Physical Integration

### Objective

Accept steering component and RT/SES interaction in isolation.

### Work

- Direct SES component test with test station.
- RT plus SES with Host/SYS/MTR simulated only as required.
...
- Confirm no synthetic SES feedback while physical SES is connected.

### Exit gate

- SES component and RT integration reports pass.
...
- Test MTR plus physical motor controller on current-limited unloaded/dynamometer
  rig.
- Add RT/SYS with SES/SEB/PWT disconnected.
