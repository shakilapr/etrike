 ...
 *
 * Comprehensive checksum validation for SES/SEB protocol messages.
 *
 * The SES/SEB checksum is: XOR(bytes[0..6]) ^ 0xFF
...
static void test_known_vector_ses_max_angle() {
    ...
    r.target_speed    = 525;        // 0x20D, max SES/SEB range
    r.roll_cnt_enable = 1;
    r.checksum_enable = 1;
...
int main() {
    printf("=== SES/SEB Checksum Full Tests ===\n\n");
