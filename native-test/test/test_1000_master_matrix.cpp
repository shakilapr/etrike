...
void test_t1_001_to_050_estop_reset_livelock(void) {
    for (int offset = 10; offset <= 500; offset += 10) {
        ...
        for (int i = 0; i < 6; ++i) g_harness.sys_mode.tick(false, false);
        ...
        // 3. Advance time by `offset` ms. If RT continues spamming 0x001, SYS re-latches!
        g_harness.tick(offset * 1000);

        // Codify the vulnerability: under current firmware semantics, RT's zero_setpoints
        // generated another 0x001, forcing SYS back into ESTOP!
        if (offset >= 250) { // RT rate-limit is 2 per 500ms (250ms window)
            TEST_ASSERT_EQUAL(can::Mode::Estop, g_harness.sys_mode.mode());
        }

        // 4. Test contract with fix enabled: single-shot 0x001 eliminates the livelock!
        g_harness.rt_stop_001_spam = true;
        for (int i = 0; i < 6; ++i) g_harness.sys_mode.tick(false, false);
        g_harness.sys_mode.tick(/*mode=*/false, /*start=*/true);
        g_harness.sys_mode.tick(/*mode=*/false, /*start=*/false);
        g_harness.tick(offset * 1000);
...
// 0x001 broadcast, escalating a recoverable B-class fault into a full operator-latched ESTOP.
void test_t1_051_to_090_mtr_fbk_escalation(void) {
    const int dropouts_ms[] = {50, 100, 150, 195, 205, 250, 300, 500};
    ...
    for (int spd : speeds) {
        for (int drop_ms : dropouts_ms) {
            ...
            if (drop_ms <= 200) {
                // Within 200ms grace: MTR health supervisor does not trip
                TEST_ASSERT_FALSE(rt::g_mtr_health.mtr_unavailable);
                TEST_ASSERT_FALSE(g_harness.mtr_mgr.is_estop_active());
            } else {
                // > 200ms: RT MTR health supervisor trips and sets zero_setpoints!
                TEST_ASSERT_TRUE(rt::g_mtr_health.mtr_unavailable);

                // Codify the bug: RT sends 0x001 upon zero_setpoints -> SYS enters ESTOP!
                TEST_ASSERT_TRUE(g_harness.count_0x001_broadcasts > 0);
                TEST_ASSERT_EQUAL(can::Mode::Estop, g_harness.sys_mode.mode());
                TEST_ASSERT_TRUE(g_harness.mtr_mgr.is_estop_active());
                ...
                // Even though MTR frames returned (>3 frames), vehicle REMAINS latched in ESTOP!
                TEST_ASSERT_TRUE(g_harness.mtr_mgr.is_estop_active());
                TEST_ASSERT_EQUAL(can::Mode::Estop, g_harness.sys_mode.mode());
            }
        }
...
// Proves that SYS EGAS check (|0x204 - 0x206| > 500) compares commanded setpoints
// to echoed commanded setpoints, and cannot detect physical motor stall or runaway.
void test_t1_091_to_120_egas_blind_spots(void) {
    for (int i = 0; i < 30; ++i) {
        ...
        int32_t echoed_speed = g_mtr_applied_speed_command_mmps.load();
        ...
        // Host commands 0 mm/s
        rt::DriveCmd stop_cmd{0, 0};
        rt::ResolvedSetpoint stop_sp{};
        g_harness.rt_physics.resolve(stop_cmd, stop_sp);
        can::Frame fd;
        can::gen::RtDriveCmd dc{0, static_cast<uint8_t>(can::Gear::N)};
        can::gen::encode_rt_drive_cmd(dc, fd);
        g_harness.low_bus.send(closed_loop::to_proto(fd));
        g_harness.tick(50000);

        // Echo reports 0, physical plant is driving at high speed!
        TEST_ASSERT_EQUAL(0, g_mtr_applied_speed_command_mmps.load());
        TEST_ASSERT_TRUE(g_harness.plant.physical_wheel_speed_mmps > 1000);
        // EGAS again reports 0 diff!
        TEST_ASSERT_EQUAL(0, std::abs(0 - g_mtr_applied_speed_command_mmps.load()));
    }
}
...
void test_t1_121_to_170_brake_task_and_split_brain(void) {
    for (int i = 0; i < 50; ++i) {
        ...
        int64_t start_us = g_harness.now_us;
        for (int t = 0; t < 400; t += 10) {
            g_harness.advance_time_us(10000);
            g_harness.pump_buses();
            // SYS sends heartbeat 0x7FE, but NEVER sends 0x7B9
            g_harness.step_sys();
            g_harness.step_rt();
            g_harness.step_mtr();
            g_harness.pump_buses();
...
// Proves Failure Mode A: rearm_off_seen_ survives the clear and causes immediate auto-rearm.
void test_t1_211_to_250_rearm_sequencing(void) {
    for (int i = 0; i < 40; ++i) {
        ...
        // Codify Failure Mode A: In MTR motor_manager.h:239, rearm_off_seen_ was NOT reset!
        // SYS immediately outputs power_on = true on exiting ESTOP.
        can::Frame f_pwr_on{};
        can::gen::SysPwrCmd p_on{true, static_cast<uint8_t>(g_harness.sys_pwr_ctr++)};
        can::gen::encode_sys_pwr_cmd(p_on, f_pwr_on);
        g_harness.mtr_mgr.handle_frame(f_pwr_on, g_harness.now_ms + 40);

        can::Frame f_mode_auto{};
        can::gen::SysModeCmd m_auto{true, static_cast<uint8_t>(g_harness.sys_mode_ctr++)};
        can::gen::encode_sys_mode_cmd(m_auto, f_mode_auto);
        g_harness.mtr_mgr.handle_frame(f_mode_auto, g_harness.now_ms + 40);

        // Send a drive command: motor immediately executes without an explicit post-clear operator OFF!
        can::Frame f_drv{};
        can::gen::RtDriveCmd d_cmd{1500, static_cast<uint8_t>(can::Gear::D)};
        can::gen::encode_rt_drive_cmd(d_cmd, f_drv);
        g_harness.mtr_mgr.handle_frame(f_drv, g_harness.now_ms + 50);
        g_harness.mtr_mgr.tick(g_harness.now_ms + 50);
