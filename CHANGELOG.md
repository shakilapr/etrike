...
### Summary
This maintenance release resolves 7 critical cross-node pipeline bugs across `sys-esp32`, `rt-esp32`, and Jetson `autoware_vehicle_bridge` under Steer-by-Wire (EPS-C) and Brake-by-Wire (SEB) integration.
...
- **Steering ESTOP Exit Coupling (`rt-esp32`)**: Decoupled `g_steering_exit_request` from periodic `0x110 SYS_MODE_CMD` frames and bound it strictly to the confirmed two-frame `SAFETY_CLEAR` sequence from `0x011`, allowing EPS-C ESTOP centering ramps to complete without interruption.
