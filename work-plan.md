...
- Unit conversion: internal mdeg ↔ SES/SEB decideg (÷100)
...
- Mock both CAN buses, HC-SR04, SES feedback
...
- Verify full pipeline: `/cmd_vel` → `0x300` → RT kinematics → `0x202` + `0x200` → SYS MCP4725 + SES angle.
- Dynamic angle clamp active.
- **Test (hardware):** Jetson commands speed=1500, yaw=200 → MCP4725 voltage correct, SES angle correct, speed clamp applies, following error monitored.
