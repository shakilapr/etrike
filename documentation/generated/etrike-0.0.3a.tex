...
buses. Actuators are \textbf{SES/SEB} CAN modules: SES (steer-by-wire) and SEB
(electro-hydraulic brake). \textbf{Mode-gated dual control (Option D):} RT
directly commands both SES and SEB in AUTO mode; SYS commands SEB in MANUAL
...
\texttt{0x169} & VCU\_SES\_REQ & RT & SES (steering) & 8 & Angle cmd +
security bytes & 50 Hz & Medium \\
\texttt{0x201} & SES\_STATUS & SES & RT & 8 & Steering angle + status feedback
& 100 Hz & Medium \\
\texttt{0x202} & SES\_ErrInfo & SES & RT & 8 & 25 fault flags (8× L3) & 10 Hz
& Medium \\
\texttt{0x203} & SES\_Version & SES & RT & 8 & SW + HW version & 1 Hz &
Lowest \\
...
Lowest \\
\texttt{0x6FA} & SES\_Test & SES & RT & 8 & Motor current, ECU temp, supply
...
\begin{quote}
\textbf{ID note}: SES/SEB units are preprogrammed and cannot be reconfigured.
SES uses factory command \texttt{0x169} and status \texttt{0x201}, plus
...
\texttt{0x204} to avoid collision with SES \texttt{0x169}. \texttt{0x205}
...
Low only & \texttt{0x201}, \texttt{0x721} & SES/SEB status feedback --- RT/SYS
...
\texttt{0x7B9} → SEB. SES standalone (RT idle). DC-DC on. Mode gated by SYS
\texttt{0x110}. \\
\textbf{AUTO} & Jetson \texttt{AckermannControlCommand} → high CAN
\texttt{0x300} → RT kinematics → low CAN \texttt{0x204} (MTR: speed+gear) +
\texttt{0x169} (SES: angle). \textbf{MTR} drives MCP4725 + gear relays
...
\item
  ...
  SES/SEB protocol specifies otherwise (see \texttt{0x169}, \texttt{0x7B9} in
  can-dictionary).
\item
  \textbf{Manual mode is pass-through, not dead.} SYS mirrors physical inputs to
  outputs.
\item
  \textbf{Actuators are standalone CAN modules.} SES, SEB, and DC-DC are
  commanded via CAN.
\item
  \textbf{RT is the only dual-bus node.} No direct Jetson ↔ SYS path.
\item
  \textbf{Listen Before Speaking.} SES/SEB units require receiving status
...
\item
  \textbf{Mode-gated dual control of SES/SEB actuators (Option D).} In AUTO, RT
  commands both SES and SEB directly --- 1-hop from the kinematics engine. In
  MANUAL, SYS commands SEB based on lever position; SES runs standalone. The
...
--- mismatch → ESTOP \\
\textbf{Why only motor needed MTR} & SES/SEB SES and SEB already do EGAS Level
...
\subsection{6.2 Mode-Gated Dual Control (Option D) --- SES/SEB
Actuators}\label{mode-gated-dual-control-option-d-SES/SEB-actuators}

The SES and SEB are commanded by different nodes depending on mode. Four
...
MANUAL brake but RT can still brake in AUTO. The SES's internal timeout (20ms
...
low-level CAN with SES/SEB's own rolling counter + checksum validation already
...
\bottomrule\noalign{}
\endlastfoot
SES (0x169) & Sends angle from kinematics & Does NOT send --- SES
standalone \\
...
RT failure → steer lost? & Yes & Same & No (SYS bridges) & Yes (SES timeout
backup) \\
...
\subsubsection{\texorpdfstring{Steering --- SES/SEB SES via CAN
(\texttt{0x169})}{Steering --- SES/SEB SES via CAN (0x169)}}\label{steering-SES/SEB-SES-via-can-0x169}
...
\textbf{Unit conversion} (internal mdeg ↔ SES/SEB decideg):
...
\textbf{SES/SEB protocol specifics:}
...
\bottomrule\noalign{}
\endlastfoot
Command ID & \texttt{0x169} (factory default --- SES/SEB preprogrammed, not
...
\textbf{Timeout fault} & If \texttt{0x169} stops for \textgreater20 ms, SES
...
\bottomrule\noalign{}
\endlastfoot
MANUAL & RT does NOT send \texttt{0x169}. SES standalone. RT still listens
...
72V gear via relays), brake control (SES/SEB SEB via CAN \texttt{0x7B9}), DC-DC
...
current mode. RT already knows the full SES/SEB SEB protocol --- this is the
...
\subsubsection{\texorpdfstring{Brake --- SES/SEB SEB via CAN
(\texttt{0x7B9})}{Brake --- SES/SEB SEB via CAN (0x7B9)}}\label{brake-SES/SEB-seb-via-can-0x7b9}
...
verified SES/SEB SEB spec: \texttt{seb\_raw\ =\ (uint8\_t)(kpa\ *\ 0.02f)}.
...
\texttt{0x204}. \\
3 & \st{SES timeout-fault behavior unknown} & \st{On ESTOP or comm loss,
...
fallback on mechanical jam. Obstacle-triggered ESTOP holds current angle. SES
...
mode only} & \textbf{RESOLVED:} Verified SES/SEB SEB spec:
...
mid-ramp, leaving SES to comm-fault at an off-center angle. A 30° steering
lock in MANUAL mode (where SES is standalone with no active centering) makes
...
18 & \textbf{SES mechanical jam silent-stop recovery path unclear} & When
...
threshold represents a 100\% error --- the SES must have ZERO response to
...
\href{docs/steering-unit.md}{\texttt{docs/steering-unit.md}} & SES/SEB SES
protocol reference \\
\href{docs/brake-unit.md}{\texttt{docs/brake-unit.md}} & SES/SEB SEB protocol
reference \\
...
\href{docs/SES/SEB-security-protocol.md}{\texttt{docs/SES/SEB-security-protocol.md}}
& Rolling counter + XOR checksum for SES/SEB actuators \\
...
noted (SES/SEB protocol uses Motorola LSB).
...
Nodes: RT ESP32-S3, SYS ESP32-S3, SES/SEB SES (steering), SES/SEB SEB (brake),
DC-DC converter.
...
\begin{quote}
Placed at \texttt{0x204} to avoid collision with SES error info frame at
\texttt{0x202}. SES/SEB units are preprogrammed and cannot be reconfigured.
\end{quote}
...
converts: \texttt{seb\_raw\ =\ (uint8\_t)(kpa\ *\ 0.02f)} (verified SES/SEB
...
\subsection{0x201 --- SES\_STATUS (SES/SEB SES
Feedback)}\label{x201-ses_status-SES/SEB-SES-feedback}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Sender} & SES/SEB SES (steering module) \\
\textbf{Receiver(s)} & RT \\
\textbf{DLC} & 8 \\
...
\texttt{physical\_deg\ =\ raw\ ×\ 0.1\ −\ 3000}. The SES encodes steering
...
torque). Raw 0 → -12.1 Nm. Raw 241 → 12.0 Nm. The SES biases torque readings
...
Both are listed --- the SES may report these in alternate frames or the CSV
represents signals available across firmware versions.
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{0x169 --- VCU\_SES\_REQ (SES/SEB SES
Command)}\label{x169-vcu_ses_req-SES/SEB-SES-command}
...
\textbf{Receiver(s)} & SES/SEB SES (steering module) \\
\textbf{DLC} & 8 \\
\textbf{Period} & 20 ms (50 Hz) --- \textbf{continuous, every frame} \\
\textbf{Endianness} & Motorola LSB (little-endian) \\
\textbf{Note} & Factory default \texttt{0x169}. SES/SEB unit is preprogrammed
...
The SES may internally separate these --- the upper nibble of byte 5 carries
...
nibble; the SES validates the security portion independently of the speed
portion.

\textbf{Slew rate:} Speed-dependent. RT computes
\texttt{VCU\_SES\_Tgt\_StrAngleSpd} based on speed to ensure smooth steering.
CSV lists range 125--525 °/s. The SES may reject speed commands below 125 °/s.
\end{quote}
...
(45500 mdeg → 30455 raw → 45.5°). Verify which encoding the SES actually
...
\subsection{0x202 --- SES\_ErrInfo (SES/SEB SES Error
Detail)}\label{x202-ses_errinfo-SES/SEB-SES-error-detail}
...
4). RT must subscribe and escalate L3 faults to ESTOP. The SES has
...
\subsection{0x203 --- SES\_Version (SES/SEB SES Firmware
Version)}\label{x203-ses_version-SES/SEB-SES-firmware-version}
...
\subsection{0x6FA --- SES\_Test (SES/SEB SES
Telemetry)}\label{x6fa-ses_test-SES/SEB-SES-telemetry}
...
\subsection{0x7B9 --- VCU\_SEB\_REQ (SES/SEB SEB Brake
Command)}\label{x7b9-vcu_seb_req-SES/SEB-seb-brake-command}
...
\textbf{Receiver(s)} & SES/SEB SEB (brake module) \\
\textbf{DLC} & 8 \\
\textbf{Period} & 20 ms (50 Hz) --- \textbf{continuous, every frame} \\
\textbf{Endianness} & Motorola LSB (little-endian) \\
\end{longtable}
...
\texttt{XOR(bytes{[}0..6{]})\ \^{}\ 0xFF} (verify against SES/SEB spec).
...
\subsection{0x721 --- SEB\_STATUS (SES/SEB SEB Brake
Feedback)}\label{x721-seb_status-SES/SEB-seb-brake-feedback}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Sender} & SES/SEB SEB (brake module) \\
...
\subsection{0x731 --- SEB\_ErrInfo (SES/SEB SEB Error
Detail)}\label{x731-seb_errinfo-SES/SEB-seb-error-detail}
...
\subsection{0x741 --- SEB\_Version (SES/SEB SEB Firmware
Version)}\label{x741-seb_version-SES/SEB-seb-firmware-version}
...
\subsection{0x6FB --- SEB\_Test (SES/SEB SEB
Telemetry)}\label{x6fb-seb_test-SES/SEB-seb-telemetry}
...
\texttt{0x169} & VCU\_SES\_REQ & RT & SES & 8 & \textbf{50 Hz} \\
\texttt{0x201} & SES\_STATUS & SES & RT & 8 & 100 Hz \\
\texttt{0x202} & SES\_ErrInfo & SES & RT & 8 & 10 Hz \\
\texttt{0x203} & SES\_Version & SES & RT & 8 & 1 Hz \\
...
\texttt{0x6FA} & SES\_Test & SES & RT & 8 & 100 Hz \\
...
Low only & \texttt{0x201} (SES/SEB SES feedback) \\
High only & \texttt{0x210}, \texttt{0x220}, \texttt{0x400} (RT telemetry) \\
Both independent & \texttt{0x7FD}, \texttt{0x7FE}, \texttt{0x7FC} (per-node
...
\texttt{0x00X}. SES/SEB IDs (\texttt{0x2XX}, \texttt{0x7XX}) are in
...
\bottomrule\noalign{}
\endlastfoot
B1 & Steering angle offset (SES CSV offset=-3000 vs architecture offset=0) &
CRITICAL \\
...
\texttt{0x169} & VCU\_SES\_REQ & RT & SES/SEB \\
\texttt{0x201} & SES\_STATUS & SES & SES/SEB \\
\texttt{0x204} & RT\_DRIVE\_CMD & RT & --- \\
\texttt{0x205} & RT\_BRAKE\_CMD & RT & --- \\
\texttt{0x302} & HOST\_LIGHT\_CMD & RT (fwd) & --- \\
\texttt{0x721} & SEB\_STATUS & SEB & SES/SEB \\
\texttt{0x7B9} & VCU\_SEB\_REQ & RT/SYS (mode-gated) & SES/SEB \\
...
The E-Trike uses a \textbf{SES/SEB SEB} electro-hydraulic brake actuator
...
Verified against SES/SEB SEB CAN protocol specification.
...
SEB\_Test), \href{brake-unit.md}{\texttt{docs/brake-unit.md}} for SES/SEB SEB
protocol reference, \href{../issues.md}{\texttt{issues.md}} M2 for the
mechanical fallback gap.}


\chapter{SES/SEB SEB --- Electro-Hydraulic Brake
Unit}\label{SES/SEB-seb-electro-hydraulic-brake-unit}
...
\begin{quote}
\textbf{Verify against SES/SEB spec} --- some units use a different constant or
a shift operation.
\end{quote}
...
CAN (Jetson + RT) and Low-Level CAN (RT + SYS + SES/SEB actuators).
...
\texttt{0x169} & VCU\_SES\_REQ & Low & RT output --- steering angle → SES \\
...
\texttt{0x201} & SES\_STATUS & Low & SES feedback --- consumed by RT on low \\
...
against the actual angle reported by SES (\texttt{0x201\ SES\_StrAngle}):
...
rod. - \textbf{Actuator faults:} SES motor failure, encoder failure, internal
control loop fault. - \textbf{CAN faults:} corrupted \texttt{0x169} frames
(caught because checksum failure → SES rejects → angle doesn't move).
...
sent to SES. This means even if Jetson commands a dangerous turn (bug, bad
...
SES mechanically accepts ±78° (its internal limit). The physical steering rack
...
\item
  Steering: \texttt{0x169} stops transmitting → SES timeout-faults.
\item
...
unverified --- hold or release). RT can still command SES. Physical brake
...
\item
  ...
  architecture §7.6 mode behavior table: ``MANUAL: RT does NOT send 0x169. SES
  standalone.''
\item
  \texttt{0x169} transmission stops \textbf{immediately} at whatever angle the
  ramp had reached.
\item
  SES: 20 ms without \texttt{0x169} → internal comm-fault timeout.
\item
  SES comm-fault behavior: unit-specific and \textbf{unknown}. May lock at
...
SES behavior after 0x169 stops & Unknown --- may \textbf{lock at that angle}
or go limp \\
\end{longtable}

At 30° steering lock in MANUAL mode, the vehicle cannot be ridden straight. The
rider must either: - Power-cycle and hope SES releases on power-loss (unknown
behavior). - Wrestle the locked steering mechanically (risk of damaging SES
...
SES timeout behavior, this creates a non-deterministic safety outcome.
...
ramp doesn't complete within 5 seconds (e.g., SES not responding), RT
...
a fixed angle, and SES will timeout-fault on 0x169 stop as it would in any
...
state because SES goes \textbf{standalone} in MANUAL mode --- the steering
...
\textbf{Alternative consideration --- hardware fix:} If SES/SEB's protocol
...
to verify with SES/SEB spec). - If SEB defaults to \textbf{hold} on power loss:
...
(if it has one --- check SES/SEB pinout) - A solenoid valve that locks hydraulic
...
CAN ID that SEB accepts as a secondary command - SES/SEB SEB must support
...
(\textasciitilde5°). - After 500ms, silent-stop as before --- SES handles the
timeout.
...
active braking (coast only) - No active steering (SES timeout-faults)
...
\textbf{Why 2000 kPa (2 MPa):} - SES/SEB SEB max pressure is 5 MPa (5000 kPa). -
...
steering (SES standalone in MANUAL) and can override brake. - No power-cycle
...
available). 3. Requires SES LBS boot sync (\textasciitilde2.5s until steering
...
covers SES 2. 0x204 staleness (200ms) --- covers motor
...
RT already sends 0x7B9 in AUTO mode with the full SES/SEB protocol (rolling
...
instead of computed pressure). No new protocol logic needed. The SES/SEB
...
\section{Issue 13: SES Mechanical Jam Silent-Stop Recovery Path
Unclear}\label{issue-13-SES-mechanical-jam-silent-stop-recovery-path-unclear}
...
fall back to silent-stop (stop transmitting \texttt{0x169}). SES enters
internal fault-lock.

§4.6 says the boot sync timeout for SES requires ``rider retry via START
...
\texttt{0x169} → SES enters comm-fault.
...
This works --- no transmission. But the SES is still fault-locked from the
...
path applies: - Short-press START → reset to \texttt{STEER\_LISTEN\_SYNC} (SES
...
SES responds to \texttt{0x201} with \texttt{SES\_INF\_Angle\_Status\ ==\ 1}: →
\texttt{STEER\_ACTIVE} in MANUAL (SES standalone --- steering wheel works
normally). 6. If SES doesn't respond (still fault-locked): remains in
...
At 25 km/h, a 4° error (commanded 5°, actual 1°) represents the SES delivering
...
ESTOP --- which it should, because the SES has lost \textasciitilde80\% of its
commanded authority.
...
\textbf{13. SES jam recovery unclear} & Low & Trivial (documentation +
...
\textbf{Steering (SES)} & Obstacle trigger: hold angle, silent-stop after
...
stop \texttt{0x169} transmission immediately, leaving SES to comm-fault at
whatever off-center angle the ramp had reached. A 30° steering lock in MANUAL
mode (where SES is standalone and provides no active centering) would make the
...
becomes fully manual (SES standalone). 6. The MANUAL indicator bulb
...
reported by SES (\texttt{0x201\ SES\_StrAngle}) at 100 Hz.
...
This catches stuck linkage, rock jams, bent tie rods, SES motor failure,
...
clamped angle: \textbf{silent-stop} (stop transmitting \texttt{0x169}). 3. SES
...
2. Continue transmitting during the ramp so SES tracks the centering. 3. Hold
...
explicitly. 4. SES timeout-faults internally.

\textbf{Recovery:} After exiting ESTOP to MANUAL, the steer SM is in
\texttt{STEER\_FAULT}. The rider has two recovery options (same as SES boot
...
neither works (SES remains fault-locked): \textbf{power-cycle} as final
recovery.
...
RT CAN TX fails & TWAI TX errors & Log, auto-recover; persistent → SES
timeout-fault \\
...
Steering still responds to handlebars (SES standalone in MANUAL). - ESTOP
...
High CAN bus-off & MCP2515 error flags & Log, auto-recover. Zero setpoints until
restored. \\
CAN frame corruption & SES/SEB checksum fail & Frame silently discarded. Rolling
...
\textbf{SES (Steering):} \textbar{} Fault \textbar{} Detection \textbar{}
Response \textbar{}
...
\textgreater20ms \textbar{} SES internal fault lock \textbar{} \textbar{} Boot
...
Max (RT takes over 0x7B9) & SES timeout* (RT still alive in AUTO) & ON (RT
...
timeout* & SES timeout* & OFF & OFF & All OFF & Reboot → MANUAL \\
...
{[}{[}SES/SEB-security-protocol{]}{]} & Rolling counter + XOR checksum for
SES/SEB actuators \\
...
\chapter{SES/SEB SES \& SEB --- Technical
Reference}\label{SES/SEB-SES-seb-technical-reference}

Extracted and cleaned from the original SES/SEB product specification PDFs
...
\section{1. SES --- Electric Power Steering
Column}\label{SES-electric-power-steering-column}

\textbf{Product:} SES/SEB steer-by-wire intelligent steering system (SES)
\textbf{Version:} A/10 (2024-11-22) \textbf{Category:} SES
...
\item
  \textbf{Protocol:} CAN\_SES (SES/SEB proprietary steer-by-wire)
\item
...
\subsubsection{VCU → SES Command (ID: 0x169, cycle: 20 ms, DLC:
8)}\label{vcu-SES-command-id-0x169-cycle-20-ms-dlc-8}
...
transmitting on hardware.} The \texttt{kSES/SEBCanOutputEnabled} flag in
\texttt{config.h} defaults to \texttt{false} for this reason.
\end{quote}

\subsubsection{SES → VCU Status (ID: 0x201, cycle: 10 ms, DLC:
8)}\label{SES-vcu-status-id-0x201-cycle-10-ms-dlc-8}
...
\subsection{1.5 SES Fault Codes}\label{SES-fault-codes}
...
\textbf{Product:} SES/SEB wire-controlled braking system (SEB) \textbf{Version:}
...
\begin{quote}
\textbf{⚠️ Same warning as SES:} verify byte layout and checksum algorithm
before transmitting.
\end{quote}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Both SES and SEB use proprietary CAN protocols with checksums and
  ...
  protocol document from SES/SEB.
\item
  \textbf{SES requires zero-calibration} when paired with a new assembly. This
...
\item
  \textbf{SES and SEB are on the private CAN bus} (TWAI1 on the ESP32-S3).
...
\item
  ...
  SES and SEB protocols may use different addressing --- verify with SES/SEB.
\item
...
\emph{Source PDFs: \texttt{given-documentation.pdf} (58 pages, SES/SEB SES +
...
bits \\
\texttt{°/s} & Degrees per second & Steering angle speed & SES ramp rate \\
...
Role: real-time physics model, steering control (SES via CAN), CAN gateway
...
\subsection{5.1 SES/SEB SES (Steering
Actuator)}\label{SES/SEB-SES-steering-actuator}
...
\subsection{5.2 SES/SEB SEB (Brake Actuator)}\label{SES/SEB-seb-brake-actuator}
...
In degraded states: - During boot (before SES/SEB modules online): \textbf{500
ms} (2 Hz heartbeats only) - If SES/SEB modules fault: \textbf{500 ms} -
...
where a 15 ms + 25 ms = 40 ms gap could trigger the SES/SEB SES 20 ms
comm-fault timeout.
...
\section{7. Issue E --- SES/SEB 20 ms timeout with 50 Hz TX
(future)}\label{issue-e-SES/SEB-20-ms-timeout-with-50-hz-tx-future}
...
\bottomrule\noalign{}
\endlastfoot
0x300 → steering (0x169 via SES/SEB SES) & \texttt{g\_steering.set\_target()}
...
control. \\
0x301 → brake (0x205 via SYS → SES/SEB SEB) & \texttt{brake\_arbitrate()} result
...
6 & SES/SEB 20 ms margin (§7) & Medium --- will cause comm faults when steering
...
RT monitors \texttt{SES\_StrAngle} from SES status (\texttt{0x201}) at 100 Hz.
Logic runs on RT:
...
\tightlist
\item
  \textbf{SES} (steering): runs an internal position loop. If you send
...
\section{SES (Steering) specifics}\label{SES-steering-specifics}
...
\bottomrule\noalign{}
\endlastfoot
Status ID & \texttt{0x201} SES\_STATUS (100 Hz from SES) \\
Position field & \texttt{SES\_StrAngle} (int16, 0.1°/bit) \\
Alignment field & \texttt{SES\_INF\_Angle\_Status} (bit 0 of status byte) \\
Command ID & \texttt{0x169} VCU\_SES\_REQ (50 Hz to SES) \\
Boot wait & 500 ms \\
Sync timeout & 2 seconds → STEER\_FAULT, remain in MANUAL \\
Mode behavior & MANUAL: do NOT send \texttt{0x169}, SES standalone. AUTO: send
...
SES/SEB actuators run an internal bootloader and self-test on power-up. During
this window:
...
\item
  Controller boots faster than SES and immediately commands 0°.
\item
  SES slews 40° at max motor speed --- handlebars whip to center.
\item
...
\tightlist
\item
  SES: if \texttt{0x169} stops for \textgreater20 ms → internal comm fault →
...
dynamic angle clamp, {[}{[}SES/SEB-security-protocol{]}{]} for rolling counter +
...
\tightlist
\item
  \textbf{Steering angle:} SES runs its own internal position loop. RT
  commands an angle target; SES controls the motor to reach it.
\item
...
\chapter{SES/SEB SES --- Steer-by-Wire
Unit}\label{SES/SEB-SES-steer-by-wire-unit}
...
\subsection{Unit conversion (internal mdeg ↔ SES/SEB raw, offset
-3000)}\label{unit-conversion-internal-mdeg-SES/SEB-raw-offset--3000}
...
Command the steering rack to a specific angle. The SES's internal PID drives
...
The SES accepts commands up to ±780° (raw ±7800). The physical trike steering
...
If the physical wheel gets stuck (rock jam, linkage failure) while the SES
tries to turn:
...
with same counter → SES rejects, triggers fault.
...
SES}\label{security-enable-bits-byte-5-unique-to-SES}

Unlike the SEB, the SES requires explicit enable bits to activate its security
checks:
...
The SES has an absolute encoder that retains calibration across power cycles.
...
\tightlist
\item
  \textbf{Required:} Only when the SES is first mated to a new mechanical
  assembly.
\item
...
only \\
SES timeout-fault behavior & TBD --- lock, center, or freewheel & Verify
against SES/SEB spec \\
\end{longtable}
...
\item
  ...
  cycles.
\item
  \textbf{Not reconfigurable.} Factory CAN IDs are fixed. SES command
  \texttt{0x169} (customized from factory default \texttt{0x200}).
  \texttt{RT\_DRIVE\_CMD} is at \texttt{0x204} to avoid collision.
\item
  \textbf{MANUAL mode:} RT does not send \texttt{0x169}. SES operates
...
\item
  \textbf{ESTOP:} RT stops sending \texttt{0x169}. SES timeout-faults
  (behavior TBD).
\end{itemize}


\chapter{SES/SEB CAN Security Protocol --- Rolling Counter +
Checksum}\label{SES/SEB-can-security-protocol-rolling-counter-checksum}

SES/SEB SES and SEB actuators require two security bytes in every command
...
\section{Why SES/SEB requires this}\label{why-SES/SEB-requires-this}
...
\section{Frame layout (both SES and
SEB)}\label{frame-layout-both-SES-and-seb}
...
\item
  ...
  1, \ldots).
\item
  Check that byte ordering matches SES/SEB's little-endian expectation.
\end{enumerate}
...
errors than CRC-8 or CRC-16. SES/SEB's design trades stronger error detection
...
SES protocol, §8.6 for SEB protocol, {[}{[}can-dictionary{]}{]} for bit-level
frame layouts.}
...
Examples from the E-Trike: - \texttt{0x201} SES\_STATUS (SES → RT): Only RT
...
except SES/SEB actuators which use \textbf{little-endian (LSB first)} per their
...
marked otherwise. SES/SEB protocols use ``Motorola LSB'' --- Motorola bit
...
except where SES/SEB factory protocols dictate little-endian.
...
CAN signals: reverse the bytes before sending. - For SES/SEB little-endian
...
\texttt{docs/steering-unit.md} for SES/SEB little-endian layout.}
...
period) needs tight periodicity --- a missed 20 ms window causes SES comm
fault.
...
The SES/SEB units require XOR-based checksums on every command frame. Same
...
SES & \texttt{0x200} every 20ms & \textasciitilde20ms & Internal comm fault \\
...
\bottomrule\noalign{}
\endlastfoot
CRC & SES/SEB checksum (\texttt{XOR\ \^{}\ 0xFF}) \\
...
Endianness & Big-endian (MSB first) for our messages; Motorola LSB for SES/SEB
units \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
Steering (SES) & \textbf{ASIL-C} & Steer-by-wire --- loss of steering control
→ severe hazard \\
...
SES comm loss & \textless50 ms & 20 ms (1 frame gap) & SES internal timeout
fault \\
...
\texttt{0x720} - Steering: SES commands stopped (timeout-faults internally) -
...
\begin{itemize}[leftmargin=*,nosep]
  \item \texttt{steering-unit.md} --- SES/SEB SES protocol reference and C++ implementation
  \item \texttt{brake-unit.md} --- SES/SEB SEB protocol reference and C++ implementation
  ...
  \item \texttt{SES/SEB-security-protocol.md} --- Rolling counter + XOR checksum
  ...
  \item \texttt{given-documentation-ocr.md} --- Source PDF extraction (SES/SEB/DCDC specs)
\end{itemize}
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item \texttt{given-documentation.pdf} --- Original SES/SEB specification PDF (58 pages)
