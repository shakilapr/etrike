"""Profile availability and controlled transitions (workplan §3.1).

Full Vehicle / Bench Test are refused outright while Phase 2 (CANalyst-II
transport) is deferred — never a silent fallback to Pure Software.
"""
...
def test_profile_change_requires_confirm(client):
    ...
    sid = ses["session_id"]
    r = client.post(f"/api/v1/sessions/{sid}/profile", json={"profile": "pure_software", "confirm": False})
...
def test_profile_change_to_physical_refused(client):
    ...
    r = client.post(
        ...
        json={"profile": "full_vehicle", "confirm": True, "expected_revision": ses["revision"]},
    )
    assert r.status_code == 503
    assert r.json()["code"] == "profile.physical_unavailable"
