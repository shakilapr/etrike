...
overridable via ``VTC_*`` environment variables through :meth:`VtcConfig.from_env`.
"""
...
class VtcConfig(BaseModel):
    ...
    title: str = "E-Trike Vehicle Test Console"
    api_prefix: str = "/api/v1"

    @classmethod
    def from_env(cls) -> "VtcConfig":
        """Build config from ``VTC_*`` environment variables, falling back to defaults."""
        overrides: dict[str, object] = {}
        if v := os.getenv("VTC_HOST"):
            overrides["host"] = v
        if v := os.getenv("VTC_PORT"):
            overrides["port"] = int(v)
        if v := os.getenv("VTC_PROFILE"):
