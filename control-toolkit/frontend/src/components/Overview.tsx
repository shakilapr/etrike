...
export function Overview() {
  ...
  // Single stream/CAN label · never join two synonyms.
  ...
  const gearLabel = signalText(drive, 'gear')
  ...
  return (
    <WorkspaceShell
      ...
      title="Overview"
      description={`Vehicle command and feedback state · session ${ses?.session_id ?? 'none'} · ${messages.length} live messages`}
      sectionLabel="Observe"
    >

      <section className="safety-strip" data-testid="safety-strip" aria-label="Safety and mode">
        {/* Multi-source ESTOP (latch + 0x001 H/L + SYS/RT), matching the topbar. */}
        <div
        ...
        >
          ...
          {/* Continuous quantity · progress bar only. */}
          <MeterBar
            value={brakeKpa}
            max={5000}
            tone="high-bad"
      ...
      <div className="cards metric-cards" data-testid="overview-meters">
        ...
        <MetricCard
          title="Wheel speed"
          valueText={physSpeed != null ? physSpeed.toFixed(0) : '—'}
          unit="mm/s"
        ...
        <MetricCard
          ...
          unit="kPa"
          sub={hostBrake ? 'High · HOST_BRAKE_REQ' : rtBrake ? 'Low · RT_BRAKE_CMD' : 'No brake command'}
          freshness={hostBrake?.freshness ?? rtBrake?.freshness ?? sebStatus?.freshness}
          value={brakeKpa}
          max={5000}
          tone="high-bad"
          testId="card-brake"
        ...
        <div className="card metric-card" data-testid="card-gear">
          ...
          {/* Single discrete value · no duplicate pill. */}
          ...
          <div className="card-sub muted">High · HOST_DRIVE_CMD 0x300</div>
        </div>
        ...
        <table className="data-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td className="mono">
                {steerDeg != null ? `${steerDeg.toFixed(1)}°` : '—'}
              </td>
              <td className="muted">not comparable</td>
              <td className="meter-cell">
            ...
            <tr>
              <td>Brake</td>
              <td className="mono">
                {brakeCmdKpa != null ? `${brakeCmdKpa.toFixed(0)} kPa` : '—'}
              </td>
              <td className="mono">
                {brakeKpa != null
            ...
            <tr>
              ...
              <td className="muted">—</td>
              <td className="meter-cell">
                {/* Binary ESTOP · pill, not a progress bar */}
                <StatusPill
                  label={
                    estopObs.any
                      ? estopObs.label
