...
RT  (rt-esp32)
  ...
  │
  ├─ Low 0x169 VCU_SES_REQ      20 ms → receiver SES (SES / steer-by-wire)
...
SES / SES — steer-by-wire unit (vendor)
...
| **0x169** | `VCU_SES_REQ` | low | 8 | 20 ms | RT → **SES** | `steering` | **rt-esp32** `steering_control` + SES encode |
...
| **SES (SES)** | **0x169** vendor command | Low steering; enables ON; angle raw; ~20 ms |
...
### Recipe C — Steer-by-wire unit command (SES / SES on 0x169)
