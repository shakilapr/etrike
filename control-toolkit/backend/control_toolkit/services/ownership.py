"""Source ownership and stimulus leases (one producer per bus + CAN ID)."""
...
class OwnershipTable:
        ...
        with self._lock:
            ...
            if existing is not None and existing.owner != owner:
                raise OwnershipConflict(
                    f"{bus} 0x{can_id:X} owned by {existing.owner}"
                )
    ...
    def release_owner(self, owner: str) -> int:
        with self._lock:
            ids = [lid for lid, l in self._by_id.items() if l.owner == owner]
            for lid in ids:
    ...
    def _expire_locked(self, now: float) -> None:
        expired = [lid for lid, l in self._by_id.items() if l.expires_at_mono <= now]
        for lid in expired:
            lease = self._by_id.pop(lid)
            key = (lease.bus, lease.can_id)
            if self._by_key.get(key) is lease:
