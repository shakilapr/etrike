...
def test_full_vehicle_allowed_without_adapter(unattached_client):
    """Real mode is enterable without CANalyst; no silent virtual fallback."""
    r = unattached_client.post("/api/v1/sessions", json={"profile": "full_vehicle"})
    assert r.status_code == 200
    ses = r.json()["session"]
...
def test_bench_test_allowed_without_adapter(unattached_client):
    r = unattached_client.post("/api/v1/sessions", json={"profile": "bench_test"})
    assert r.status_code == 200
    ses = r.json()["session"]
    assert ses["profile"] == "bench_test"
    assert ses["destination"] == "physical"
...
def test_profile_change_requires_confirm(client):
    ...
    r = client.post(
        ...
        json={"profile": "bench_test", "confirm": False},
    )
...
def test_profile_change_to_physical_without_adapter(unattached_client):
    ses = unattached_client.post("/api/v1/sessions", json={"profile": "bench_test"}).json()[
        "session"
    ]
    sid = ses["session_id"]
    r = unattached_client.post(
        f"/api/v1/sessions/{sid}/profile",
        json={
